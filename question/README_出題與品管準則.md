...
> - **國語自動補題**：課文僅自 `knowledge/1_課綱研究/國語/<學期>/<版本>/` 之 **KL4「單課研究紀錄」** 內 **「課文全文錄製」** 抽取；該課須另有 **KL4「考古題與討論」**。不足時腳本略過並註記 **`【資料不齊備】`**；請依 **`knowledge/1_課綱研究/國語/KL3_國語_研究進度_課文與索引.md`** 將該課研究檔補完整後再產題（不使用獨立 `課文原文` 說明檔或 `.txt` 備援目錄）。
...
> - 詳見 `knowledge/3_考古題/README_考古題蒐集規範與來源索引.md` §一第 5-6 條。
...
- **全檔控管**：正解為唯一最長選項（嚴格大於其餘3項，含並列不算）的比例 > 40% → 觸發 BIAS 阻擋
- **BIAS 為上架硬門檻（不分 QL3／QL4）**：此 40% 門檻是所有上架題的必要條件，與 QL 等級脫鉤（見 `README_驗證與盲測準則.md` §4.6）。程式 `evaluate_question_quality.js` 已於 2026-07 對齊：全科目全年級統一 40% 門檻、「正解＝唯一最長」判定為嚴格大於其餘 3 選項（含並列不算），無科目豁免（JOB-272）。
...
## 附錄 A：圖片依賴題型的處理規範（JOB-212，2026-04-28）

現行 Eidos 平台為純文字選擇題，不支援圖片附件。出題前須判定該單元知識點能否用純文字完整表達：

| 適性 | 判定標準 | 處置 |
|:--|:--|:--|
| 🟢 可出 | 核心知識點可用純文字、算式或情境文字完整表達 | 正常出題 |
| 🟡 部分可出 | 部分子題型需圖片，其他子題型可純文字 | 只出純文字部分；圖片相關子題封存 |
| 🔴 封存 | 核心活動依賴圖形操作、刻度讀數、數格子等 | 整單元暫緩出題；於 KL4 標記 `image_required: true` |

各科目具體判定清單見各科 KL4 研究紀錄（`knowledge/1_課綱研究/[科目]/`）。
