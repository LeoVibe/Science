...
> **上版前驗證**（UI ↔ 資料一致性、人工瀏覽器抽測）見 [`docs/上版前驗證標準.md`](../docs/上版前驗證標準.md)。盲測（本文件）與上版前驗證是兩個層次：盲測驗證題目本身的題意清晰度，上版前驗證題目在前端顯示的正確性。兩者都是釋出前的必要條件。
...
| 達 QL4（素材達 RM3 ＋盲測通過＋三大檢查） | `is_publishable: true`（正式上架，見 §4.6） |
| 未跑盲測但達 QL3（素材達 RM3、通過基礎檢查、`review_status` 非 pending、無爭議） | `is_publishable: true`（Beta 上架，見 §4.6） |
| 僅達 QL2（素材達 RM2、通過基礎檢查、`review_status` 非 pending） | `is_publishable: true`（Alpha 上架，見 §4.6） |
| 未達 QL2（QL1） | `is_publishable: false` |
...
| **RM** 研究成熟度 | 單課研究狀態 | RM0-RM4 | `knowledge/README_研究架構總綱.md` |
...
每題 QL 由以下條件**累積**判定，以「具體可驗證條件」為準。**素材天花板（RM）為前置條件**：RM 決定該題 QL 的上限，素材不足者即使盲測通過也不得升上限以上的 QL（見 `knowledge/README_研究架構總綱.md`）。

| QL | 必要條件 | 素材天花板（前置）| 具體可驗證 |
|:--:|:--|:--:|:--|
| **QL1** | 僅憑學科常識出題，無課程錨點 | RM1 | 該課無課綱元素對應；無課文亦無考古題 |
| **QL2** | 題目對應課綱課程元素 | RM2 | 該課存在課綱對應元素（KL4 單課研究紀錄之課程元素章節）|
| **QL3** | QL2 ＋（考古題 **或** 課文之一）＋通過基礎檢查 | RM3 | 該課 KL4 考古題與討論或課文全文錄製至少其一非空 |
| **QL4** | QL3 ＋ 盲測通過 ＋ 三大檢查 | RM3 | 素材達 RM3 ＋ 題目 `blind_evaluation === true` |
| **QL5** | QL4 + 專家認證 | RM3＋ | `verifying_model` 含 `Expert` + 有 `commonMisconception`（未來）|

**等級累積原則**：達 QL4 的題目同時也算 QL3、QL2。
**素材天花板說明**：課文不再是必要門票——**考古題與課文地位對等**，有任一個即達 RM3，兩個都有為 RM4（超額素材）。RM3 是 QL3／QL4 的共同素材天花板門檻。

#### QL3／QL4 的檢查機制（基礎檢查／三大檢查定義）

| 檢查項 | 內容 | 生效級別 |
|:--|:--|:--|
| 洩答清除 | 選項無「這也是作者想強調的…」等出題後設評語 | QL3 起（基礎）|
| 長度 BIAS | 該課「正解＝唯一最長」比例 ≤ 40%（不分 QL3／QL4，**上架硬門檻**，見 §4.6）| QL3 起（基礎）|
...
> **基礎檢查** ＝ 洩答清除／長度 BIAS／文本錯位，QL3 上架前必過。
> **三大檢查** ＝ 題幹吻合度／選項隨機度／長度 BIAS，QL4 升級時驗。
> 長度 BIAS 兩級都管，是上架硬門檻（見 §4.6）。

### 4.3 每題 QL 語意說明（對外品質標籤）

> 本表文字為全專案 QL 對外標籤的基礎定義，與前台 `AboutView.tsx`「如何定義題庫品質」一致。其他文件（出題準則、研究總綱、進度彙整、產品介紹）引用時須採同一套用語；技術判定條件見 §4.2。

| 等級 | 名稱 | 說明 |
|:--:|:--|:--|
| **QL1** | 常識題 | 僅憑學科常識產出，無課程錨點，學習目標不夠精準。 |
| **QL2** | 課綱對應 | 題目對應該課課綱課程元素；素材尚未經課文或考古題實證。 |
| **QL3** | 實證參考 | 該課具考古題或課文之一作為實證素材；誘答設計有實證基礎。 |
| **QL4** | 盲測淬煉 | 用不同 Agent 進行盲測且通過；題幹與選項經實測精修。 |
...
**上架三級制**（2026-06-16，v2 課文非必要改版）：
- **QL4 庫 → 正式上架**（前端無標記）
- **QL3 庫 → Beta 上架**（前端標「Beta · 尚未嚴謹測試」）
- **QL2 庫 → Alpha 上架**（前端標「Alpha · 最早期實驗，素材未經實證」）
- **QL1 以下 → 不上架**

> QL2 以 Alpha 標籤露出，是使用者拍板的品質取捨：承擔「無實證素材題露給學生」的成本，換取拿不到課文／考古題的課也能先上線。素材天花板（RM）為各級的前置門檻——QL3／QL4 需該課素材達 RM3（考古題或課文之一）。

**BIAS 硬門檻（不分 QL3／QL4，所有上架題必過）**：該課「正解＝唯一最長選項」（正解字數嚴格大於其餘 3 個選項，含並列不算）比例 > 40% → 不得上架（`is_publishable` 不得設 true）。全科目全年級統一適用，無豁免。BIAS 是題目結構品質，與 QL 等級脫鉤——QL3 與 QL4 都須通過此驗證（學生不得能用「選最長」猜對）。校正方式：盲測時同步重寫過短的錯誤選項以平衡長度（不改正解／答案／題意），再以 `evaluate_question_quality.js` 二次驗證（2026-07 起腳本已對齊本條文，見 JOB-272）。
...
> **is_publishable 是獨立的上架閘門，不是 QL 的自動結果**：QL2 達標（素材達 RM2、通過基礎檢查、`review_status` 非 pending）即可設 true（標 Alpha）；QL3 設 true 標 Beta、QL4 設 true 正式上架；任何題即使 QL4，發現特殊問題（答案爭議、不當內容、多次回報等）可單題設 false 獨立下架，不影響同課其他題。詳見 §2.5。
