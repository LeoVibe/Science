...
async function callLLM(prompt, currentQuestionsSize, totalNeeded, filePath = "") {
    ...
    if (isMath) {
        ...
        extraFocus = "設計強調觀察邏輯、實驗步驟與生態常識的情境題";
        outlineDoc = "《00_自然科共同發展總綱》";
    } else if (isEnglish) {
        subjectExpertise = "小學雙語教育";
        extraFocus = "將英文詞彙與句型無縫融入真實生活對話場景";
        outlineDoc = "《00_英語科共同發展總綱》（或參考跨科原則）";
    }
    ...
    try {
        const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=${apiKey}`, {
            ...
            body: JSON.stringify({
                systemInstruction: {
                    parts: [{
                        text: `你是一位頂尖的${subjectExpertise}心理學專家。
你的任務是熟讀${outlineDoc}所確立的跨學科標準，並根據「大腦友善三原則（同理心投射、合理化迷思、語氣延展）」與「中年級 4-4-2 認知動態配比」來${extraFocus}。
長情境題幹必須使用【在xxx時】標籤。
        ...
        if (parsed.new_questions && Array.isArray(parsed.new_questions)) {
            parsed.new_questions.forEach(q => {
                if (q.options && typeof q.answer_index === 'number') {
                    ...
                    const newAnswerIndex = finalOptions.indexOf(correctAnswer);
...
async function processFile(filePath) {
    ...
    console.log(`📊 現存優質題: ${currentCount} / 目標: ${TARGET_QUESTIONS} (需補足: ${neededCount})`);

    // 準備 Prompt
    const prompt = `
目前我們正在擴充與修復【${jsonObj.publisher}版】第【${jsonObj.grade}】年級第【${jsonObj.semester}】學期，課名：《${jsonObj.lesson_title || jsonObj.lesson_id}》。
原題庫已經有 ${currentCount} 題優質題，我們還需要 ${neededCount} 題。
請根據這篇課文（或依據你對國小課文《${jsonObj.lesson_title}》的既有知識庫），為我自動生成這 ${neededCount} 題。
...
請回傳包含 ${neededCount} 個物件的 new_questions 陣列 JSON。`;

    const newQuestions = await callLLM(prompt, currentCount, neededCount, filePath);
