#!/bin/bash
# JOB-246 Task 6 全量 Serial Dispatch（單 worker 順序跑，避免 quota 爆炸）
#
...
queues = {p: json.load(open(f'scripts/jobs/JOB-246/_full_targets_{p}.json', encoding='utf-8')) for p in publishers}
merged = []
