#!/bin/bash
# JOB-231 Phase 0.3 Pilot 5 份 dispatch（四下_自然）
# 並行 3 條 codex 跑（避免一次 5 條過載；剩餘 2 條等前批完成）
# 對齊黃金樣本 schema：knowledge/3_考古題/3_L2_結構化抽取/_golden_samples/四下_自然_<Phase 0.2 確定>.json

set -u

OUT_DIR="knowledge/3_考古題/3_L2_結構化抽取/四下/四下_自然_pilot"
LOG_DIR="scripts/jobs/JOB-231/_pilot_logs"
PROMPT_TPL="scripts/jobs/JOB-231/A1_pilot_prompt_template_natural_g4.md"
...
PILOT_LIST=(
  "翰林_110_員林國小_第一次段考|knowledge/3_考古題/2_MD淬鍊文字_整合版/四下/四下_自然_翰林/翰林_110_員林國小_第一次段考.md|翰林-codex_only+paper_full+answer_full（主流情境）"
  "康軒_108_安慶國小_第一次段考|knowledge/3_考古題/2_MD淬鍊文字_整合版/四下/四下_自然_康軒/康軒_108_安慶國小_第一次段考.md|康軒-codex_only+paper_full+answer_full"
  "翰林_111_勝利國小_期中考|knowledge/3_考古題/2_MD淬鍊文字_整合版/四下/四下_自然_翰林/翰林_111_勝利國小_期中考.md|翰林-dual_source（稀有情境、answer_partial 邊界）"
  "康軒_113_2_期末考|knowledge/3_考古題/2_MD淬鍊文字_整合版/四下/四下_自然_康軒/康軒_113_2_期末考.md|康軒-claude_only+paper_full+answer_full"
  "南一_112_新北桃子腳國小_期末考|knowledge/3_考古題/2_MD淬鍊文字_整合版/四下/四下_自然_南一/南一_112_新北桃子腳國小_期末考.md|南一-codex_only+paper_full+answer_full（題量大補位）"
)
...
python3 << 'PY'
import json, glob, os
with open('knowledge/3_考古題/3_L2_結構化抽取/_meta/science_codes_legal_II.json') as f:
    legal = json.load(f)
legal_set = set([c['code'] for c in legal['performance']] + [c['code'] for c in legal['content']])
total_violations = 0
for path in sorted(glob.glob('knowledge/3_考古題/3_L2_結構化抽取/四下/四下_自然_pilot/*.json')):
    try:
