#!/usr/bin/env python3
"""
JOB-231 Phase B：跨全量 137 份四下_自然 JSON 驗證編碼合法性、階段、重複。
讀目錄：_golden_samples/<本 JOB 黃金>.json (1) + 四下_自然_pilot/* (5) + 四下_自然_*出版社/* (~131)
寫報告：knowledge/3_考古題/3_L2_結構化抽取/_validation_report_natural_g4.json
不修改原始 JSON。
"""
...
EXPECTED_FILES = 124  # 1 黃金 + 5 Pilot + 118 全量
EXPECTED_LEGAL_CODES = 75
STAGE_MARK = "Ⅱ"  # Ⅱ
DETAIL_LIMIT = 30

ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../.."))
DATA_ROOT = os.path.join(ROOT, "knowledge/3_考古題/3_L2_結構化抽取")
LEGAL_CODES_PATH = os.path.join(DATA_ROOT, "_meta/science_codes_legal_II.json")
REPORT_PATH = os.path.join(DATA_ROOT, "_validation_report_natural_g4.json")

# 黃金樣本路徑於 Phase 0.2 確定後填入
INPUT_GLOBS = [
    os.path.join(DATA_ROOT, "_golden_samples/四下_自然_*.json"),  # Phase 0.2 產出後 glob 自動命中
    os.path.join(DATA_ROOT, "四下/四下_自然_pilot/*.json"),
    os.path.join(DATA_ROOT, "四下/四下_自然_翰林/*.json"),
    os.path.join(DATA_ROOT, "四下/四下_自然_康軒/*.json"),
    os.path.join(DATA_ROOT, "四下/四下_自然_南一/*.json"),
]
