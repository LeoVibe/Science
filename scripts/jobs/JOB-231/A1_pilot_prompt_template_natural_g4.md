# JOB-231 Phase 4 Codex Pilot Prompt 模板（四下_自然）

> 此模板由 dispatch 腳本注入 `{EXAM_ID}` / `{MD_PATH}` / `{OUTPUT_PATH}` 後送給 codex exec。
> 派工單依據：`jobs/JOB-231-AG-四下-自然-考古題L2結構化抽取.md`
...
你是 Codex，負責對一份四下_自然 整合版 MD 試卷做 **Phase A1 + A2 整合抽取**：
...
- `黃金樣本（唯一 schema 範例）`：
  - **只能讀**：`knowledge/3_考古題/3_L2_結構化抽取/_golden_samples/四下_自然_翰林_108_四維國小_第二次段考.json`（自然四下主黃金樣本，50 題、57 codes、distinct 8/75（單元考集中合理）、編碼合法率 100%、Claude Opus 4.7 親做）
  - **❌ 嚴禁參考**：`knowledge/3_考古題/3_L2_結構化抽取/_meta/social_codes_legal_II.json`（社會科編碼清單，與本任務無關）
  - **❌ 嚴禁參考**：`_golden_samples/_archive_social/` 與 `_golden_samples/四下_社會_*` 任何社會科樣本（code prefix 如 Aa/Bb/Cb/Dc 等不適用自然科）
...
```json
{
  "_meta": {
    ...
    "schema_version": "1.0",
    "extracted_at": "<ISO 時間>",
    "extractor": "Codex (gpt-5.5) - JOB-231 Pilot",
    "_inheritance": {
...
1. **❌ 禁止使用不在 75 條合法清單的 code。** 自然科第 Ⅱ 階段合法 prefix 為 performance（ti/tr/tc/tm/po/pe/pa/pc/ai/ah/an）+ content（INa/INb/INc/INd/INe/INf/INg）。其他 prefix 一律不合法。
2. **❌ 禁止使用其他學習階段（Ⅰ/Ⅲ/Ⅳ/Ⅴ）的編碼。** 四下_自然 = 第 Ⅱ 學習階段，code 必須以 `-Ⅱ-` 中綴。
3. **✅ 找不到合適編碼時**，留 `codes_candidate: []` 並在題目欄加 `"extract_skipped": "no_matching_code"`，**不可硬塞**。

### Reason 品質

4. **❌ 禁止寫「相關」「對應」「與此題有關」等空泛字眼。** 必須引用題幹原句。
5. **✅ 範例（合格）**：`"題幹「電池正極接小燈泡」直接對應「常見電路與導體絕緣體」概念。"`
6. **❌ 範例（不合格）**：`"與此題相關"`、`"考電的知識"`。
...
- [ ] 全部 codes_candidate.code 在 75 條合法清單內（檢查 science_codes_legal_II.json）
...
1. 讀 `{MD_PATH}` 與 `science_codes_legal_II.json`
2. 解析 MD frontmatter 寫入 `_meta`（注意 semester 改為「四下」）
3. 解析「## 試卷」與「## 答案」段落，逐題抽結構化
4. 對每題從 75 條中找 1-3 條最匹配的 code（四下_自然主題：電的世界 / 力與運動 / 動物的身體 / 認識動物 / 月亮 / 水的奧妙 / 簡單機械等）
5. 寫 `_summary` 統計
6. 自查清單跑一遍，發現違規修正
