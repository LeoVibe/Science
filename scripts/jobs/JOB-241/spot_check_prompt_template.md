# JOB-231 Phase 5 Spot Check Prompt Template（四下_自然）

> 由 Claude wakeup 階段呼叫。每次注入一個 worker + rank 的資訊，由 codex 對該 JSON 做嚴格 review，append 結果到 `scripts/jobs/JOB-231/_spot_check.log`。
...
> 標準調整說明：JOB-228 採 ≥5 字，發現對短題幹（≤10 字）易誤判 fail。JOB-229 改 ≥3 字並驗證通過；JOB-231 沿用 ≥3 字。
...
執行：
```bash
echo '{"worker":"{WORKER}","rank":{RANK_NUM},"exam_id":"{EXAM_ID}","publisher":"{PUBLISHER}","verdict":"...","checks":{"schema":"...","reason":"...","distribution":"...","cognitive":"...","illegal":"..."},"note":"<一句話結論>","timestamp":"<ISO 時間>"}' >> scripts/jobs/JOB-231/_spot_check.log
```

把 `...` 替換成實際結果。`note` 寫一句話結論（如「結構與黃金樣本一致，reason 全引用題幹」或「編碼分布偏斜：Ca-Ⅲ-1 占 35%」）。
