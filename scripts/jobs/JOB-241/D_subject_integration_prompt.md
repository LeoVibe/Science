# JOB-241 Phase D：六下_國語全科目整合 _L2_整合.md 派工 Prompt

> 此 prompt 在 Phase C（三份 _L2_summary.md）完成後，派 codex 寫全科目整合報告。

## 輸入

- `knowledge/3_考古題/3_L2_結構化抽取/六下/六下_國語_翰林/_L2_summary.md`
- `knowledge/3_考古題/3_L2_結構化抽取/六下/六下_國語_康軒/_L2_summary.md`
- `knowledge/3_考古題/3_L2_結構化抽取/六下/六下_國語_南一/_L2_summary.md`
...
---
`last_updated`: <ISO 時間>
`updated_by`: Codex (gpt-5.5) - JOB-241 Phase D
`scope`: 六下_國語（翰林+康軒+南一）
`total_files`: 90
`total_questions`: <int>
`total_codes`: <int>
---
...
本檔以三份版本級 `_L2_summary.md` 為主，並以 `_validation_report_chinese_g6.json` 中六下_國語三版本的 `per_file` 切片核對總題數與 codes 數。
...
請讀取 `knowledge/3_考古題/3_L2_結構化抽取/_meta/chinese_codes_legal_III.json` 的 `content` 陣列，依其中的 `category` 欄位分群，建立表格：
...
請讀取 `_meta/chinese_codes_legal_III.json` 的 `performance` 陣列，依 prefix（`{數字}-Ⅱ` 部分）分群，建立表格：
