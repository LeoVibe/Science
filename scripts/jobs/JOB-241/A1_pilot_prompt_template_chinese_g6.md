# JOB-241 Phase 0.3 Codex Pilot Prompt 模板（六下_國語）

> 此模板由 dispatch 腳本注入 `{EXAM_ID}` / `{MD_PATH}` / `{OUTPUT_PATH}` 後送給 codex exec。
> 派工單依據：`jobs/JOB-241-AG-六下-國語-考古題L2結構化抽取.md`
...
你是 Codex，負責對一份六下_國語 整合版 MD 試卷做 **Phase A1 + A2 整合抽取**：
...
- `EXAM_ID`：`{EXAM_ID}`
- `MD 整合檔`：`{MD_PATH}`
- `合法編碼清單`：`knowledge/3_考古題/3_L2_結構化抽取/_meta/chinese_codes_legal_III.json`（65 條：31 學習表現 + 34 學習內容）
- `黃金樣本（唯一 schema 範例）`：
  - **只能讀**：`knowledge/3_考古題/3_L2_結構化抽取/_golden_samples/六下_國語_翰林_*.json`（國語六下主黃金樣本，Claude Sonnet 4.6 親做、編碼合法率 100%）
  ...
  - **❌ 嚴禁參考**：任何社會科或自然科黃金樣本
  - **❌ 嚴禁參考**：任何 `-Ⅰ-` 或 `-Ⅱ-` 或 `-Ⅳ-` 或 `-Ⅴ-` 編碼（六下使用第 Ⅲ 學習階段）
  - 國語科 code 格式**僅限**：
    - performance（學習表現）：`{數字}-Ⅲ-{數字}`，如 `1-Ⅲ-1`、`5-Ⅲ-3`（共 34 條）
    - content（學習內容）：`{大寫字母組合}-Ⅲ-{數字}`，如 `Aa-Ⅲ-1`、`Bb-Ⅲ-4`（共 34 條）
  - **若你看到 `-Ⅲ-`、`ti-`、`INa-`、`1a/2a`（社會科/自然科 prefix）→ 立即停止並重讀 chinese_codes_legal_III.json**
  - **若你看到 `-Ⅰ-` 或 `-Ⅱ-` 中綴 → 立即停止，六下國語必須用 `-Ⅲ-` 中綴**
...
```json
{
  "_meta": {
    ...
    "schema_version": "1.0",
    "extracted_at": "<ISO 時間>",
    "extractor": "Codex (gpt-5.5) - JOB-241 Pilot",
    "_inheritance": {
...
1. **❌ 禁止使用不在 65 條合法清單的 code。** 國語科第 Ⅲ 階段合法格式：performance（`{數字}-Ⅲ-{數字}`）+ content（`{大寫字母組合}-Ⅲ-{數字}`）。其他格式一律不合法。
2. **❌ 禁止使用其他學習階段（Ⅰ/Ⅲ/Ⅳ/Ⅴ）的編碼。** 六下_國語 = 第 Ⅲ 學習階段，code 必須以 `-Ⅲ-` 中綴。
...
- [ ] 全部 codes_candidate.code 在 65 條合法清單內（檢查 chinese_codes_legal_III.json）
- [ ] 全部 code 含 `-Ⅲ-` 中綴（無任何 `-Ⅲ-` 或 `-Ⅰ-`）
...
1. 讀 `{MD_PATH}` 與 `chinese_codes_legal_III.json`
2. 解析 MD frontmatter 寫入 `_meta`（注意 semester 為「六下」、subject 為「國語」）
3. 解析「## 試卷」與「## 答案」段落，逐題抽結構化
4. 對每題從 65 條中找 1-3 條最匹配的 code（六下_國語主題：注音符號/字形辨識 / 詞語理解與應用 / 句型與造句 / 閱讀理解與段落大意 / 修辭與文學表現 / 標點符號 / 寫作與語文應用）
5. 寫 `_summary` 統計
6. 自查清單跑一遍，發現違規修正
