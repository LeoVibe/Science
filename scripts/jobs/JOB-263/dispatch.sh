#!/bin/bash
# JOB-263 四下國語 KL4考古題淬煉 serial dispatch（Codex訂閱制）
...
PD="scripts/jobs/JOB-263/_prompts"
LOG="scripts/jobs/JOB-263/_logs"
BASE="knowledge/1_課綱研究/國語/四下"
mkdir -p "$LOG"
ONLY="${1:-}"

echo "=== JOB-263 四下國語KL4淬煉 start $(date '+%H:%M:%S') ==="
PASS=0; FAIL=0
> "$LOG/fail_list.txt"
for p in "$PD"/翰林_L*.txt "$PD"/康軒_L*.txt "$PD"/南一_L*.txt; do
  [ -f "$p" ] || continue
  key=$(basename "$p" .txt)               # 如 翰林_L10
  [ -n "$ONLY" ] && [ "$key" != "$ONLY" ] && continue
  pub="${key%%_*}"; L="${key#*_}"
  # 找該課_new路徑+課名
  rec=$(ls "$BASE/$pub/KL4_四下_${pub}_${L}_"*_單課研究紀錄.md 2>/dev/null | head -1)
  [ -z "$rec" ] && { echo "[$key] 無單課研究紀錄,跳過"; continue; }
  name=$(basename "$rec" | sed "s/KL4_四下_${pub}_${L}_//;s/_單課研究紀錄.md//")
  newf="$BASE/$pub/KL4_四下_${pub}_${L}_${name}_考古題與討論_new.md"
  ...
  # 驗證
  if python3 scripts/jobs/JOB-263/verify_kl4.py "$newf" "$name" 2>&1; then
    PASS=$((PASS+1))
  else
