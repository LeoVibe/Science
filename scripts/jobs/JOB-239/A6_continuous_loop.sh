#!/bin/bash
# JOB-239 Phase 5 continuous full loop wrapper（worker A/B/C 通用）
# 反覆呼叫 dispatch 直到該 worker ~38 份全部完成或進度停滯。
#
# 用法：
#   bash scripts/jobs/JOB-239/A6_continuous_loop.sh A   # 跑 worker A
#   bash scripts/jobs/JOB-239/A6_continuous_loop.sh B   # 跑 worker B
#   bash scripts/jobs/JOB-239/A6_continuous_loop.sh C   # 跑 worker C
...
while true; do
  ...
  if ! bash scripts/jobs/JOB-239/A5_full_dispatch.sh "$WORKER"; then
