#!/usr/bin/env python3
"""
JOB-243 Phase 1 — A4: 合併 partial → alignment_raw.json
...
OUT_PATH = os.path.join(ROOT, 'knowledge/3_考古題/3_L2_結構化抽取/三下/alignment/alignment_raw.json')
...
def main():
    ...
    def load_l2(exam_id):
        ...
        # 找 L2 JSON 檔
        for sub in ['三下_國語_pilot', '三下_國語_翰林', '三下_國語_康軒', '三下_國語_南一']:
            p = os.path.join(ROOT, f'knowledge/3_考古題/3_L2_結構化抽取/三下/{sub}/{exam_id}.json')
            if os.path.exists(p):
                d = json.load(open(p))
                l2_q_cache[exam_id] = {q['question_id']: q for q in d['questions']}
                return l2_q_cache[exam_id]
        # 嘗試 golden_samples
        p = os.path.join(ROOT, f'knowledge/3_考古題/3_L2_結構化抽取/_golden_samples/三下_國語_{exam_id}.json')
        if os.path.exists(p):
            d = json.load(open(p))
            l2_q_cache[exam_id] = {q['question_id']: q for q in d['questions']}
            return l2_q_cache[exam_id]
    ...
    raw = {
        '_meta': {
            ...
            'merger': 'A4_merge.py - JOB-243 Phase 1',
