# JOB-243 Phase 1：L2 ↔ KL3/KL4 自動對齊（三下_國語，spec v1.1）

你是 Codex（gpt-5.5），負責對單一試卷做 L3 對齊。

> **JOB-243 重點**：本試卷已預先篩選為「新版（current）」範圍：
> - 翰林：112-113 學年（111 及以前為舊版，已排除）
> - 康軒：111-112 學年（110 及以前為舊版，已排除）
> - 南一：111-113 學年（110 及以前為舊版，已排除）
>
> 三下/四下 改版年份不同：四下翰林 110→111，三下翰林 111→112；四下康軒/南一 全新版，三下康軒/南一 110→111。
...
4. **KL4 單課研究紀錄（36 份，含 RC-01 課文）**：
   - `knowledge/1_課綱研究/國語/三下/翰林/KL4_三下_翰林_L*_單課研究紀錄.md`（12 份）
   - `knowledge/1_課綱研究/國語/三下/康軒/KL4_三下_康軒_L*_單課研究紀錄.md`（12 份）
   - `knowledge/1_課綱研究/國語/三下/南一/KL4_三下_南一_L*_單課研究紀錄.md`（12 份）
...
```json
{
  "_meta": {
    ...
    "extractor": "Codex (gpt-5.5) - JOB-243 Phase 1",
    "total_questions": <int>
  },
  "l2_to_kl_links": [
    {
      ...
      "kl3_links": [
        {
          ...
          "evidence": "題幹中『拔不起來的筆』一詞與第二段引用"
        }
      ],
      "kl4_links": [
        {
          "kecode": "0130201",
          "kl3_anchor": "kl3-0130201-XXXXXXXXXX",
          "rc01_evidence": true,
          "rc01_quote": "潔白的牆壁上，被人寫個大大的「大」字"
        }
      ],
...
8. **R2 防 false positive**：課程名稱 ≥3 字連續匹配，但若課名通用度高（如「許願」「玩玩具」「下雨的時候」），題幹同段須含至少 1 個課文特有詞（從 KL4 §關鍵詞彙 抓）才採用
9. **version_match 必填**（v1.1 新增）：
   - 預設 `current`（本試卷已通過外部篩選為新版範圍）
   - 若題幹意外引用**三下舊版專屬課文**（清單見下方），標 `version_match: "legacy"` + `legacy_lesson_title` 填課文名 + `kl4_links` 為空陣列
...
- [ ] kl4_links 課碼格式正確（7 碼，版本碼對，年級為 3）
