"""JOB-265 六下國語 KL4 考古題淬煉 — 生成每課 codex prompt（10題）。
...
KL3 = f'{BASE}/KL3_六下_國語_研究總綱.md'
L2 = 'knowledge/3_考古題/3_L2_結構化抽取/六下/六下_國語_L2_整合.md'
PD = os.path.join(ROOT, 'scripts/jobs/JOB-265/_prompts')
os.makedirs(PD, exist_ok=True)

TPL = '''你是國語科題型研究專家。為六下{pub} {L}《{name}》設計 10 道「課文題型示例與誘答分析」，接受雙重驗證。
...
count = 0
for pub in ['翰林', '康軒', '南一']:
    for rec in sorted(glob.glob(f'{BASE}/{pub}/KL4_六下_{pub}_L*_單課研究紀錄.md')):
        m = re.search(rf'KL4_六下_{pub}_(L\d+)_(.+?)_單課研究紀錄', os.path.basename(rec))
        if not m: continue
        L, name = m.group(1), m.group(2)
        out = f'{BASE}/{pub}/KL4_六下_{pub}_{L}_{name}_考古題與討論_new.md'
