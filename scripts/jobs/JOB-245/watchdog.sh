#!/bin/bash
# JOB-245 Phase 1 watchdog — 偵測卡住的 codex 並 kill
# 邏輯：每 60s 檢查 worker_C_南一.log 是否有 update，若 > 15 min 無變化且仍有 codex 跑 → kill
# 跑：bash scripts/jobs/JOB-245/watchdog.sh > scripts/jobs/JOB-245/_phase1_logs/watchdog.log 2>&1 &

LOG_FILE="scripts/jobs/JOB-245/_phase1_logs/worker_C_南一.log"
...
while true; do
  ...
  if [ "$current_size" != "$last_size" ]; then
    ...
    if [ $elapsed -gt $STALE_THRESHOLD ]; then
      ...
      # 簡化：直接 kill 所有 JOB-245 相關 codex
      pids=$(ps aux | grep "codex exec" | grep "JOB-245" | grep -v grep | awk '{print $2}')
      for pid in $pids; do
