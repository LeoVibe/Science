#!/bin/bash
# JOB-245 Phase 1 Pilot dispatch（六下_國語，新版 5 份試跑）
# 六下改版年份：翰林 111→112 / 康軒 110→111 / 南一 110→111
...
L2_FULL_DIR="knowledge/3_考古題/3_L2_結構化抽取/六下/六下_國語_翰林"
L2_KH_DIR="knowledge/3_考古題/3_L2_結構化抽取/六下/六下_國語_康軒"
L2_NY_DIR="knowledge/3_考古題/3_L2_結構化抽取/六下/六下_國語_南一"
...
import os, json
base = 'knowledge/3_考古題/3_L2_結構化抽取/六下'
# 六下 Pilot 5 份固定清單（翰林無 113）
fixed = [
    ('翰林', '112', '六下_國語_翰林', '翰林_112_2_期末考'),
    ('翰林', '112', '六下_國語_翰林', '翰林_112_內安國小_第一次段考'),
    ('康軒', '112', '六下_國語_康軒', '康軒_112_勝利國小_期中考'),
    ('南一', '112', '六下_國語_南一', '南一_112_2_期末考'),
    ('南一', '113', '六下_國語_南一', '南一_113_02第一次評量五年級國語科評量試卷_未知'),
]
...
echo "=== JOB-245 Phase 1 Pilot start ==="
...
for path in sorted(glob.glob('knowledge/3_考古題/3_L2_結構化抽取/六下/alignment/_partial/alignment_partial_*.json')):
    try:
...
echo "  - 六下_國語 預期 R1 比例略低於四下（顯式引用較少），R3 比例會高"
echo ""
