#!/usr/bin/env python3
"""
JOB-233 Phase B：跨全量 121 份五下_自然 JSON 驗證編碼合法性、階段、重複。
讀目錄：_golden_samples/<本 JOB 黃金>.json (1) + 五下_自然_pilot/* (5) + 五下_自然_*出版社/* (~115)
寫報告：knowledge/3_考古題/3_L2_結構化抽取/_validation_report_natural_g5.json
不修改原始 JSON。
"""
...
EXPECTED_FILES = 121  # 1 黃金 + 5 Pilot + 115 全量
EXPECTED_LEGAL_CODES = 89  # science_codes_legal_III.json
STAGE_MARK = "Ⅲ"
DETAIL_LIMIT = 30

ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../.."))
DATA_ROOT = os.path.join(ROOT, "knowledge/3_考古題/3_L2_結構化抽取")
LEGAL_CODES_PATH = os.path.join(DATA_ROOT, "_meta/science_codes_legal_III.json")
REPORT_PATH = os.path.join(DATA_ROOT, "_validation_report_natural_g5.json")

# 黃金樣本路徑於 Phase 0.2 確定後填入
INPUT_GLOBS = [
    os.path.join(DATA_ROOT, "_golden_samples/五下_自然_*.json"),  # Phase 0.2 產出後 glob 自動命中
    os.path.join(DATA_ROOT, "五下/五下_自然_pilot/*.json"),
    os.path.join(DATA_ROOT, "五下/五下_自然_翰林/*.json"),
    os.path.join(DATA_ROOT, "五下/五下_自然_康軒/*.json"),
    os.path.join(DATA_ROOT, "五下/五下_自然_南一/*.json"),
]
...
def confidence_value(candidate):
    ...
    if isinstance(confidence, str):
        # 字串 confidence (high/medium/low) 排序：high=3, medium=2, low=1
