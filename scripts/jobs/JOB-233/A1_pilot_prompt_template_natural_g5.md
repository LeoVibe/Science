# JOB-233 Phase 4 Codex Pilot Prompt 模板（五下_自然）

> 此模板由 dispatch 腳本注入 `{EXAM_ID}` / `{MD_PATH}` / `{OUTPUT_PATH}` 後送給 codex exec。
> 派工單依據：`jobs/JOB-233-AG-五下-自然-L2結構化抽取.md`
...
你是 Codex，負責對一份五下_自然 整合版 MD 試卷做 **Phase A1 + A2 整合抽取**：
...
- `黃金樣本（唯一 schema 範例）`：
  - **只能讀**：`knowledge/3_考古題/3_L2_結構化抽取/_golden_samples/五下_自然_翰林_*.json`（自然五下主黃金樣本，Claude Sonnet 4.6 親做、編碼合法率 100%；若尚未存在則讀同目錄其他 JOB-233 黃金樣本）
  - **❌ 嚴禁參考**：`knowledge/3_考古題/3_L2_結構化抽取/_meta/social_codes_legal_III.json`（社會科編碼清單，與本任務無關）
  - **❌ 嚴禁參考**：任何社會科黃金樣本（code prefix 如 1a/1b/Aa/Ab/Ba 等不適用自然科）
  - **❌ 嚴禁參考**：任何 `-Ⅱ-` 編碼（四下以下使用，五下使用第Ⅲ學習階段）
  - 自然科 code prefix **僅限**：
    - performance（學習表現）：`ti / tr / tc / tm / po / pe / pa / ah / ai / an`，共 10 個，全部以 `-Ⅲ-` 中綴
    - content（學習內容）：`INa / INb / INc / INd / INe / INf / INg`，共 7 個 prefix，全部以 `-Ⅲ-` 中綴
  - **若你看到 `1a / 1b / 1c / 2a / 2b / Aa / Ab / Ac / Ba / Bb / Ca / Da / Db / Dc` 等 prefix → 立即停止並重讀 science_codes_legal_III.json**
  - **若你看到 `-Ⅱ-` 中綴 → 立即停止，五下自然必須用 `-Ⅲ-` 中綴**
...
```json
{
  "_meta": {
    ...
    "schema_version": "1.0",
    "extracted_at": "<ISO 時間>",
    "extractor": "Codex (gpt-5.5) - JOB-233 Pilot",
    "_inheritance": {
...
4. 對每題從 89 條中找 1-3 條最匹配的 code（五下_自然主題：物質與粒子 / 能量轉換 / 電與磁 / 生物與環境 / 生態系 / 地球與宇宙 / 科技應用等）
5. 寫 `_summary` 統計
6. 自查清單跑一遍，發現違規修正
