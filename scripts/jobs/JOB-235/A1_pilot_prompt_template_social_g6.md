# JOB-235 Phase 4 Codex Pilot Prompt 模板（六下_社會）

> 此模板由 dispatch 腳本注入 `{EXAM_ID}` / `{MD_PATH}` / `{OUTPUT_PATH}` 後送給 codex exec。
> 派工單依據：`jobs/JOB-235-AG-六下-社會-考古題-L2-結構化抽取.md`
...
你是 Codex，負責對一份六下_社會 整合版 MD 試卷做 **Phase A1 + A2 整合抽取**：
...
- `黃金樣本（唯一 schema 範例）`：
  - **只能讀**：`knowledge/3_考古題/3_L2_結構化抽取/_golden_samples/六下_社會_翰林_108_四維國小_第一次段考.json`（社會六下主黃金樣本，Claude Sonnet 4.6 親做、編碼合法率 100%）
  ...
  - **❌ 嚴禁參考**：任何 `_II_` 編碼（四下以下使用，六下使用第Ⅲ學習階段）
  - 社會科 code prefix **僅限**：
...
```json
{
  "_meta": {
    ...
    "schema_version": "1.0",
    "extracted_at": "<ISO 時間>",
    "extractor": "Codex (gpt-5.5) - JOB-235 Pilot",
    "_inheritance": {
...
2. **❌ 禁止使用其他學習階段（Ⅰ/Ⅱ/Ⅳ/Ⅴ）的編碼。** 六下_社會 = 第 Ⅲ 學習階段，code 必須以 `-Ⅲ-` 中綴。
...
4. 對每題從 46 條中找 1-3 條最匹配的 code（六下_社會主題：臺灣近代史 / 日治時期 / 戰後臺灣發展 / 民主化與人權 / 憲法與法律 / 政府組織與選舉 / 地理環境與人文 / 文化多樣性）
5. 寫 `_summary` 統計
6. 自查清單跑一遍，發現違規修正
