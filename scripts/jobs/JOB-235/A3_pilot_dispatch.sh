#!/bin/bash
# JOB-235 Phase 0.3 Pilot 5 份 dispatch（六下_社會）
# 並行 3 條 codex 跑（避免一次 5 條過載；剩餘 2 條等前批完成）
# 對齊黃金樣本 schema：knowledge/3_考古題/3_L2_結構化抽取/_golden_samples/六下_社會_翰林_108_四維國小_第一次段考.json

set -u

OUT_DIR="knowledge/3_考古題/3_L2_結構化抽取/六下/六下_社會_pilot"
LOG_DIR="scripts/jobs/JOB-235/_pilot_logs"
PROMPT_TPL="scripts/jobs/JOB-235/A1_pilot_prompt_template_social_g6.md"

mkdir -p "$OUT_DIR" "$LOG_DIR"
> "$LOG_DIR/timing.csv"

# Phase 0.3 Pilot 5 候選（多元覆蓋，全部 answer_full）：
# 1-2: 翰林 不同年份（108/109），paper_full+answer_full
# 3-4: 康軒 不同情境（ocr_corrected / 純 codex_only）
# 5: 南一 paper_full+answer_full+ocr_corrected
# 格式：EXAM_ID|MD_PATH|CATEGORY

PILOT_LIST=(
  "翰林_108_田中國小_第一次段考|knowledge/3_考古題/2_MD淬鍊文字_整合版/六下/六下_社會_翰林/翰林_108_田中國小_第一次段考.md|翰林-108-paper_full+answer_full+codex_only+ocr_corrected+columns_reordered"
  "翰林_109_海佃國小_第一次段考|knowledge/3_考古題/2_MD淬鍊文字_整合版/六下/六下_社會_翰林/翰林_109_海佃國小_第一次段考.md|翰林-109-paper_full+answer_full+codex_only"
  "康軒_108_大園國小_第一次段考|knowledge/3_考古題/2_MD淬鍊文字_整合版/六下/六下_社會_康軒/康軒_108_大園國小_第一次段考.md|康軒-108-paper_full+answer_full+codex_only+ocr_corrected"
  "康軒_108_成功國小_第一次段考|knowledge/3_考古題/2_MD淬鍊文字_整合版/六下/六下_社會_康軒/康軒_108_成功國小_第一次段考.md|康軒-108-paper_full+answer_full+codex_only"
  "南一_108_中正國小_第一次段考|knowledge/3_考古題/2_MD淬鍊文字_整合版/六下/六下_社會_南一/南一_108_中正國小_第一次段考.md|南一-108-paper_full+answer_full+codex_only+ocr_corrected+columns_reordered"
)
...
total_violations = 0
for path in sorted(glob.glob('knowledge/3_考古題/3_L2_結構化抽取/六下/六下_社會_pilot/*.json')):
    try:
