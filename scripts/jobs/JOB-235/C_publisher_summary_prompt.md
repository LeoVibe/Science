# JOB-235 Phase C：版本級 _L2_summary.md 派工 Prompt（六下_社會）
...
- `{OUTPUT_PATH}`: `knowledge/3_考古題/3_L2_結構化抽取/六下/六下_社會_{PUBLISHER}/_L2_summary.md`

---

## 任務內容

請對 `{PUBLISHER}` 出版社的所有六下_社會 L2 結構化抽取結果做版本級彙整，寫入 `{OUTPUT_PATH}`。

### Step 1：讀取資料

1. **驗證報告**：`knowledge/3_考古題/3_L2_結構化抽取/_validation_report_social_g6.json` 的 `per_file` 區段中 publisher = `{PUBLISHER}` 的所有 entry。
2. **逐份 JSON**：`knowledge/3_考古題/3_L2_結構化抽取/六下/六下_社會_{PUBLISHER}/*.json` + 黃金樣本（若 publisher=該黃金版本）+ Pilot（若該 publisher 有 Pilot）。
...
---
`last_updated`: <ISO 時間>
`updated_by`: Codex (gpt-5.5) - JOB-235 Phase C
`version`: {PUBLISHER}
`n_files`: {N_FILES}
---

# {PUBLISHER}六下社會 L2 版本級摘要

本摘要彙整同版目錄下所有 `*.json`，並以 `_validation_report_social_g6.json` 的 `per_file` 題數與 codes 數作交叉核對。
