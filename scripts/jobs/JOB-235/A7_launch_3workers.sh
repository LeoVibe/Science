#!/bin/bash
# JOB-235 Phase 5 啟動三 worker（A/B/C 並行 ~120 份序列抽取 → 扣黃金+Pilot 後 ~114 份）
...
echo "[$(date '+%H:%M:%S')] === JOB-235 Phase 5 啟動 3 worker ==="

# 預先建立 progress.json（避免 loop wrapper count_remaining 失敗永遠 sleep）
for W in A B C; do
  P="scripts/jobs/JOB-235/_full_progress_${W}.json"
  if [ ! -f "$P" ]; then
...
for W in A B C; do
  LOG_FILE="$LOG_DIR/JOB-235-loop-${W}.log"
  echo "[$(date '+%H:%M:%S')] 啟動 Worker $W → $LOG_FILE"
  nohup bash scripts/jobs/JOB-235/A6_continuous_loop.sh "$W" \
    > "$LOG_FILE" 2>&1 &
  echo "  PID=$!"
...
echo "  python3 scripts/jobs/JOB-235/dashboard.py --since-minutes 60"
echo "  tail -f $LOG_DIR/JOB-235-loop-A.log"
echo "  tail -f $LOG_DIR/JOB-235-loop-B.log"
echo "  tail -f $LOG_DIR/JOB-235-loop-C.log"
echo ""
echo "停止指令："
