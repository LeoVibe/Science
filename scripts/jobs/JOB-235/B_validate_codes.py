#!/usr/bin/env python3
"""
JOB-235 Phase B：跨全量 118 份六下_社會 JSON 驗證編碼合法性、階段、重複。
讀目錄：_golden_samples/<本 JOB 黃金>.json (1) + 六下_社會_pilot/* (5) + 六下_社會_*出版社/* (~112)
寫報告：knowledge/3_考古題/3_L2_結構化抽取/_validation_report_social_g6.json
不修改原始 JSON。
"""
...
EXPECTED_FILES = 118  # 1 黃金 + 5 Pilot + 112 全量
EXPECTED_LEGAL_CODES = 46  # social_codes_legal_III.json
STAGE_MARK = "Ⅲ"
DETAIL_LIMIT = 30

ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../.."))
DATA_ROOT = os.path.join(ROOT, "knowledge/3_考古題/3_L2_結構化抽取")
LEGAL_CODES_PATH = os.path.join(DATA_ROOT, "_meta/social_codes_legal_III.json")
REPORT_PATH = os.path.join(DATA_ROOT, "_validation_report_social_g6.json")

# 黃金樣本路徑於 Phase 0.2 確定後填入
INPUT_GLOBS = [
    os.path.join(DATA_ROOT, "_golden_samples/六下_社會_*.json"),  # Phase 0.2 產出後 glob 自動命中
    os.path.join(DATA_ROOT, "六下/六下_社會_pilot/*.json"),
    os.path.join(DATA_ROOT, "六下/六下_社會_翰林/*.json"),
    os.path.join(DATA_ROOT, "六下/六下_社會_康軒/*.json"),
    os.path.join(DATA_ROOT, "六下/六下_社會_南一/*.json"),
]
