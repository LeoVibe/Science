"""JOB-264 五下國語 KL4 考古題淬煉 — 生成每課 codex prompt（10題）。
輸出到 _new 暫存（KL4_..._考古題與討論_new.md），驗證通過才覆蓋空殼正式檔。
"""
import os, glob, re

ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..'))
BASE = 'knowledge/1_課綱研究/國語/五下'
KL3 = f'{BASE}/KL3_五下_國語_研究總綱.md'
L2 = 'knowledge/3_考古題/3_L2_結構化抽取/五下/五下_國語_L2_整合.md'
PD = os.path.join(ROOT, 'scripts/jobs/JOB-264/_prompts')
os.makedirs(PD, exist_ok=True)

TPL = '''你是國語科題型研究專家。為五下{pub} {L}《{name}》設計 10 道「課文題型示例與誘答分析」，接受雙重驗證。
...
count = 0
for pub in ['翰林', '康軒', '南一']:
    for rec in sorted(glob.glob(f'{BASE}/{pub}/KL4_五下_{pub}_L*_單課研究紀錄.md')):
        m = re.search(rf'KL4_五下_{pub}_(L\d+)_(.+?)_單課研究紀錄', os.path.basename(rec))
        if not m: continue
        L, name = m.group(1), m.group(2)
        out = f'{BASE}/{pub}/KL4_五下_{pub}_{L}_{name}_考古題與討論_new.md'
