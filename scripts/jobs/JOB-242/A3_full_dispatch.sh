#!/bin/bash
# JOB-242 Phase 1 全量 dispatch（64 份新版，3 worker 並行）
...
START=$(date +%s)
echo "=== JOB-242 Phase 1 全量 dispatch start at $(date) ==="
echo ""

# 3 worker 並行
run_worker "A_翰林" "scripts/jobs/JOB-242/_full_targets_A_翰林.json" > "$LOG_DIR/worker_A_翰林.log" 2>&1 &
PID_A=$!
run_worker "B_康軒" "scripts/jobs/JOB-242/_full_targets_B_康軒.json" > "$LOG_DIR/worker_B_康軒.log" 2>&1 &
PID_B=$!
run_worker "C_南一" "scripts/jobs/JOB-242/_full_targets_C_南一.json" > "$LOG_DIR/worker_C_南一.log" 2>&1 &
PID_C=$!
