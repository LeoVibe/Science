#!/usr/bin/env python3
"""
JOB-242 Phase 1 — A4: 合併 partial → alignment_raw.json
...
PARTIAL_DIR = os.path.join(ROOT, 'knowledge/3_考古題/3_L2_結構化抽取/四下/alignment/_partial')
OUT_PATH = os.path.join(ROOT, 'knowledge/3_考古題/3_L2_結構化抽取/四下/alignment/alignment_raw.json')

# KL4 翰林 12 課 (從檔名抓)
def build_kl4_index():
    kl4 = {}  # {(publisher, lesson): {'title': str, 'kecode': str, 'kl3_anchor': str}}
    for pub in ['翰林', '康軒', '南一']:
        d = os.path.join(ROOT, f'knowledge/1_課綱研究/國語/四下/{pub}/')
...
def main():
    ...
    def load_l2(exam_id):
        ...
        # 找 L2 JSON 檔
        for sub in ['四下_國語_pilot', '四下_國語_翰林', '四下_國語_康軒', '四下_國語_南一']:
            p = os.path.join(ROOT, f'knowledge/3_考古題/3_L2_結構化抽取/四下/{sub}/{exam_id}.json')
            if os.path.exists(p):
                d = json.load(open(p))
                l2_q_cache[exam_id] = {q['question_id']: q for q in d['questions']}
                return l2_q_cache[exam_id]
        # 嘗試 golden_samples
        p = os.path.join(ROOT, f'knowledge/3_考古題/3_L2_結構化抽取/_golden_samples/四下_國語_{exam_id}.json')
        if os.path.exists(p):
            d = json.load(open(p))
            l2_q_cache[exam_id] = {q['question_id']: q for q in d['questions']}
            return l2_q_cache[exam_id]
    ...
    raw = {
        '_meta': {
            ...
            'merger': 'A4_merge.py - JOB-242 Phase 1',
