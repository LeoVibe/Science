#!/bin/bash
# JOB-232 Phase 5 Codex Full Dispatcher (worker A/B/C) — 五下_社會
# 序列跑單一 worker 的 ~38 份；每份產出後寫 progress 並做 schema + 編碼合法率驗證。
#
# 用法：
#   bash scripts/jobs/JOB-232/A5_full_dispatch.sh A           # 跑 worker A 全部
#   bash scripts/jobs/JOB-232/A5_full_dispatch.sh A --rank 3  # 只跑 worker A 第 3 份
#   bash scripts/jobs/JOB-232/A5_full_dispatch.sh A --fresh   # 重置 progress
...
TARGETS_FILE="scripts/jobs/JOB-232/_full_targets_${WORKER}.json"
PROMPT_TEMPLATE="scripts/jobs/JOB-232/A1_pilot_prompt_template_social_g5.md"
PROGRESS_FILE="scripts/jobs/JOB-232/_full_progress_${WORKER}.json"
LOG_DIR="scripts/orchestrator-logs"
LEGAL_CODES="knowledge/3_考古題/3_L2_結構化抽取/_meta/social_codes_legal_III.json"

mkdir -p "$LOG_DIR" \
  "knowledge/3_考古題/3_L2_結構化抽取/五下/五下_社會_翰林" \
  "knowledge/3_考古題/3_L2_結構化抽取/五下/五下_社會_康軒" \
  "knowledge/3_考古題/3_L2_結構化抽取/五下/五下_社會_南一"
...
for ((i=1; i<=COUNT; i++)); do
  ...
  LOG_FILE="$LOG_DIR/JOB-232-full-${WORKER}-rank$i.log"
