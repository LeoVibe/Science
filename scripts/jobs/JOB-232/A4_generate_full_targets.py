#!/usr/bin/env python3
"""JOB-232 Phase 5 全量 targets 產生器
從 120 份五下_社會整合 MD 中扣除 1 黃金 + 5 Pilot = 114 份分到 A/B/C 三 worker。
...
def main():
    targets = []
    for pub in ['翰林','康軒','南一']:
        pub_dir = f'knowledge/3_考古題/2_MD淬鍊文字_整合版/五下/五下_社會_{pub}'
        for f in sorted(glob.glob(f'{pub_dir}/*.md')):
            ...
            if not fm:
                ...
                continue
            output_path = f'{OUT_BASE}/五下_社會_{pub}/{exam_id}.json'
            targets.append({
                'exam_id': exam_id,
                'publisher': pub,
                'md_path': f,
    ...
    for w in ['A','B','C']:
        out = {
            '_meta': {
                'phase': f'JOB-232 Phase 5 - 全量 worker {w}',
                'worker': w,
                'created_at': now,
                'total': len(workers[w]),
                'purpose': f'JOB-232 Phase 5 派 codex worker {w} 跑（總 {len(targets)} 份分 A/B/C 三 worker）'
            },
            'targets': workers[w]
        }
        out_path = f'scripts/jobs/JOB-232/_full_targets_{w}.json'
