#!/usr/bin/env python3
"""
JOB-232 Phase B：跨全量 120 份五下_社會 JSON 驗證編碼合法性、階段、重複。
讀目錄：_golden_samples/<本 JOB 黃金>.json (1) + 五下_社會_pilot/* (5) + 五下_社會_*出版社/* (~114)
寫報告：knowledge/3_考古題/3_L2_結構化抽取/_validation_report_social_g5.json
不修改原始 JSON。
"""
...
EXPECTED_FILES = 120  # 1 黃金 + 5 Pilot + 114 全量
...
REPORT_PATH = os.path.join(DATA_ROOT, "_validation_report_social_g5.json")

# 黃金樣本路徑於 Phase 0.2 確定後填入
INPUT_GLOBS = [
    os.path.join(DATA_ROOT, "_golden_samples/五下_社會_*.json"),  # Phase 0.2 產出後 glob 自動命中
    os.path.join(DATA_ROOT, "五下/五下_社會_pilot/*.json"),
    os.path.join(DATA_ROOT, "五下/五下_社會_翰林/*.json"),
    os.path.join(DATA_ROOT, "五下/五下_社會_康軒/*.json"),
    os.path.join(DATA_ROOT, "五下/五下_社會_南一/*.json"),
]
