# JOB-232 Phase 4 Codex Pilot Prompt 模板（五下_社會）

> 此模板由 dispatch 腳本注入 `{EXAM_ID}` / `{MD_PATH}` / `{OUTPUT_PATH}` 後送給 codex exec。
> 派工單依據：`jobs/JOB-232-AG-五下-社會-L2結構化抽取.md`
...
你是 Codex，負責對一份五下_社會 整合版 MD 試卷做 **Phase A1 + A2 整合抽取**：
...
- `EXAM_ID`：`{EXAM_ID}`
- `MD 整合檔`：`{MD_PATH}`
- `合法編碼清單`：`knowledge/3_考古題/3_L2_結構化抽取/_meta/social_codes_legal_III.json`（46 條：20 學習表現 + 26 學習內容）
- `黃金樣本（唯一 schema 範例）`：
  - **只能讀**：`knowledge/3_考古題/3_L2_結構化抽取/_golden_samples/五下_社會_翰林_108_四維國小_第一次段考.json`（社會五下主黃金樣本，Claude Sonnet 4.6 親做、編碼合法率 100%）
  - **❌ 嚴禁參考**：`knowledge/3_考古題/3_L2_結構化抽取/_meta/science_codes_legal_III.json`（自然科編碼清單，與本任務無關）
  - **❌ 嚴禁參考**：任何自然科黃金樣本（code prefix 如 ti/tr/po/INa 等不適用社會科）
  - **❌ 嚴禁參考**：任何 `_II_` 編碼（四下以下使用，五下使用第Ⅲ學習階段）
  - 社會科 code prefix **僅限**：
    - performance（學習表現）：`1a / 1b / 1c / 2a / 2b / 2c / 3a / 3b / 3c`，共 9 個，全部以 `-Ⅲ-` 中綴
    - content（學習內容）：`Aa / Ab / Ac / Ad / Ba / Bb / Bc / Ca / Cb / Cc / Cd / Da / Db / Dc`，共 14 個，全部以 `-Ⅲ-` 中綴
  - **若你看到 `ti / tr / tc / tm / po / pe / pa / ah / ai / an / INa / INb / INc / INd / INe / INf / INg` 等 prefix → 立即停止並重讀 social_codes_legal_III.json**
  - **若你看到 `-Ⅱ-` 中綴 → 立即停止，五下社會必須用 `-Ⅲ-` 中綴**
...
```json
{
  "_meta": {
    ...
    "schema_version": "1.0",
    "extracted_at": "<ISO 時間>",
    "extractor": "Codex (gpt-5.5) - JOB-232 Pilot",
    "_inheritance": {
...
1. **❌ 禁止使用不在 46 條合法清單的 code。** 社會科第 Ⅲ 階段合法 prefix 為 performance（1a/1b/1c/2a/2b/2c/3a/3b/3c）+ content（Aa/Ab/Ac/Ad/Ba/Bb/Bc/Ca/Cb/Cc/Cd/Da/Db/Dc）。其他 prefix 一律不合法。
2. **❌ 禁止使用其他學習階段（Ⅰ/Ⅱ/Ⅳ/Ⅴ）的編碼。** 五下_社會 = 第 Ⅲ 學習階段，code 必須以 `-Ⅲ-` 中綴。
3. **✅ 找不到合適編碼時**，留 `codes_candidate: []` 並在題目欄加 `"extract_skipped": "no_matching_code"`，**不可硬塞**。

### Reason 品質

4. **❌ 禁止寫「相關」「對應」「與此題有關」等空泛字眼。** 必須引用題幹原句。
5. **✅ 範例（合格）**：`"題幹「蔣渭水以非武裝抗爭爭取臺灣人權益」直接對應「臺灣社會運動與民主發展」的歷史脈絡。"`
6. **❌ 範例（不合格）**：`"與此題相關"`、`"考社會知識"`。
...
- [ ] 全部 codes_candidate.code 在 46 條合法清單內（檢查 social_codes_legal_III.json）
...
1. 讀 `{MD_PATH}` 與 `social_codes_legal_III.json`
2. 解析 MD frontmatter 寫入 `_meta`（注意 semester 為「五下」）
3. 解析「## 試卷」與「## 答案」段落，逐題抽結構化
4. 對每題從 46 條中找 1-3 條最匹配的 code（五下_社會主題：臺灣近代史 / 日治時期 / 戰後臺灣發展 / 民主化與人權 / 憲法與法律 / 政府組織與選舉 / 地理環境與人文 / 文化多樣性）
5. 寫 `_summary` 統計
6. 自查清單跑一遍，發現違規修正
