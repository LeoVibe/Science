#!/usr/bin/env python3
"""
JOB-236 Phase B：跨全量 91 份六下_自然 JSON 驗證編碼合法性、階段、重複。
讀目錄：_golden_samples/<本 JOB 黃金>.json (1) + 六下_自然_pilot/* (5) + 六下_自然_*出版社/* (~85)
寫報告：knowledge/3_考古題/3_L2_結構化抽取/_validation_report_natural_g6.json
不修改原始 JSON。
"""
...
EXPECTED_FILES = 91  # 1 黃金 + 5 Pilot + 85 全量
...
REPORT_PATH = os.path.join(DATA_ROOT, "_validation_report_natural_g6.json")

# 黃金樣本路徑於 Phase 0.2 確定後填入
INPUT_GLOBS = [
    os.path.join(DATA_ROOT, "_golden_samples/六下_自然_*.json"),  # Phase 0.2 產出後 glob 自動命中
    os.path.join(DATA_ROOT, "六下/六下_自然_pilot/*.json"),
    os.path.join(DATA_ROOT, "六下/六下_自然_翰林/*.json"),
    os.path.join(DATA_ROOT, "六下/六下_自然_康軒/*.json"),
    os.path.join(DATA_ROOT, "六下/六下_自然_南一/*.json"),
]
