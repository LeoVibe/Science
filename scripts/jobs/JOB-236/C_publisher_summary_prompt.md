# JOB-236 Phase C：版本級 _L2_summary.md 派工 Prompt（六下_自然）
...
- `{OUTPUT_PATH}`: `knowledge/3_考古題/3_L2_結構化抽取/六下/六下_自然_{PUBLISHER}/_L2_summary.md`

---

## 任務內容

請對 `{PUBLISHER}` 出版社的所有六下_自然 L2 結構化抽取結果做版本級彙整，寫入 `{OUTPUT_PATH}`。

### Step 1：讀取資料

1. **驗證報告**：`knowledge/3_考古題/3_L2_結構化抽取/_validation_report_natural_g6.json` 的 `per_file` 區段中 publisher = `{PUBLISHER}` 的所有 entry。
2. **逐份 JSON**：`knowledge/3_考古題/3_L2_結構化抽取/六下/六下_自然_{PUBLISHER}/*.json` + 黃金樣本（若 publisher=該黃金版本）+ Pilot（若該 publisher 有 Pilot）。
3. **合法編碼清單**：`knowledge/3_考古題/3_L2_結構化抽取/_meta/science_codes_legal_III.json`（46 條）。
...
---
`last_updated`: <ISO 時間>
`updated_by`: Codex (gpt-5.5) - JOB-236 Phase C
`version`: {PUBLISHER}
`n_files`: {N_FILES}
---

# {PUBLISHER}六下自然 L2 版本級摘要

本摘要彙整同版目錄下所有 `*.json`，並以 `_validation_report_natural_g6.json` 的 `per_file` 題數與 codes 數作交叉核對。
