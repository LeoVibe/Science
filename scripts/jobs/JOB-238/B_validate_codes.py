#!/usr/bin/env python3
"""
JOB-238 Phase B：跨全量 121 份四下_國語 JSON 驗證編碼合法性、階段、重複。
讀目錄：_golden_samples/<本 JOB 黃金>.json (1) + 四下_國語_pilot/* (5) + 四下_國語_*出版社/* (~115)
寫報告：knowledge/3_考古題/3_L2_結構化抽取/_validation_report_chinese_g4.json
不修改原始 JSON。
"""
...
REPORT_PATH = os.path.join(DATA_ROOT, "_validation_report_chinese_g4.json")

INPUT_GLOBS = [
    os.path.join(DATA_ROOT, "_golden_samples/四下_國語_*.json"),
    os.path.join(DATA_ROOT, "四下/四下_國語_pilot/*.json"),
    os.path.join(DATA_ROOT, "四下/四下_國語_翰林/*.json"),
    os.path.join(DATA_ROOT, "四下/四下_國語_康軒/*.json"),
    os.path.join(DATA_ROOT, "四下/四下_國語_南一/*.json"),
]
