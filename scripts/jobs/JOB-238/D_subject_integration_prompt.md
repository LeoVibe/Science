# JOB-238 Phase D：四下_國語全科目整合 _L2_整合.md 派工 Prompt
...
---
`last_updated`: <ISO 時間>
`updated_by`: Codex (gpt-5.5) - JOB-238 Phase D
`scope`: 四下_國語（翰林+康軒+南一）
`total_files`: 121
`total_questions`: <int>
`total_codes`: <int>
---
...
本檔以三份版本級 `_L2_summary.md` 為主，並以 `_validation_report_chinese_g4.json` 中四下_國語三版本的 `per_file` 切片核對總題數與 codes 數。
