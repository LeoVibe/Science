"""JOB-249 翰林三下自然出題 — 生成 4 課 codex prompt。
重排：L3=天氣觀測、L4=動物構造（對齊課綱與 KL4 檔名）。
每課 prompt 指示 codex 讀 KL4 素材+迷思，原創 50 題寫入 _new.json。
"""
import os

ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '..', '..'))
KL4DIR = 'knowledge/1_課綱研究/自然/三下/翰林'
...
PROMPT_DIR = os.path.join(ROOT, 'scripts/jobs/JOB-249/_prompts')
os.makedirs(PROMPT_DIR, exist_ok=True)

# 重排後：L3 title=觀測天氣(天氣)、L4 title=動物的身體(動物)
LESSONS = [
    ('L1', 1, '蔬菜園地', '植物種植與生長'),
    ('L2', 2, '水和冰', '水與物質變化'),
    ('L3', 3, '觀測天氣', '天氣觀測與解析'),
...
SCHEMA_EXAMPLE = '''{
  "meta": {"grade": "G3", "semester": "S2", "subject": "SCI", "publisher": "HANLIN", "lesson": "%(L)s", "order": %(order)d, "title": "%(name)s", "theme": ""},
  "publisher": "HanLin",
  "questions": [
    {
...
TEMPLATE = '''你是國小三年級自然科命題專家。請為「翰林版 三年級下學期 自然 {L}《{name}》（主題：{theme}）」**原創 50 道**四選一單選題。

## 步驟一：先讀完以下素材再出題
1. {kl4dir}/KL4_三下_翰林_{L}_{theme}_單課研究紀錄.md ← 知識點地圖與守衛點（命題骨架）
2. {kl4dir}/KL4_三下_翰林_{L}_{theme}_考古題與討論.md ← 考古題分析
3. {misc} ← 迷思診斷報告，挑出與本課（{theme}）相關、翰林欄有命中的迷思
...
for L, order, name, theme in LESSONS:
    out = f'{QDIR}/G3_S2_SCI_HANLIN_{L}_new.json'
    ...
    with open(os.path.join(PROMPT_DIR, f'{L}.txt'), 'w', encoding='utf-8') as f:
        f.write(prompt)
    print(f'✓ {L}.txt ({len(prompt)} chars) — {name}/{theme}')
