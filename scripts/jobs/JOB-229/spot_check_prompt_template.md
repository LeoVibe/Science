# JOB-229 Phase 5 Spot Check Prompt Template（自然科）

> 由 Claude wakeup 階段呼叫。每次注入一個 worker + rank 的資訊，由 codex 對該 JSON 做嚴格 review，append 結果到 `scripts/jobs/JOB-229/_spot_check.log`。

## Placeholder

呼叫前 sed 替換：
- `{WORKER}`: 對應 worker（A/B/C）
- `{RANK_NUM}`: worker-local rank 序號（1-39）
...
- **nonspecific**：泛指（如「考察自然概念」「測試認知能力」「與此題相關」）

> 標準調整說明：JOB-228 採 ≥5 字，發現對短題幹（≤10 字）易誤判 fail。JOB-229 改 ≥3 字。
...
> 自然科特殊性：植物生長/毛細現象兩主題易主導本卷，主題集中合理；單碼仍應分散到 INb-Ⅱ-6/INb-Ⅱ-7/INe-Ⅱ-11 等多碼。
...
執行：
```bash
echo '{"worker":"{WORKER}","rank":{RANK_NUM},"exam_id":"{EXAM_ID}","publisher":"{PUBLISHER}","verdict":"...","checks":{"schema":"...","reason":"...","distribution":"...","cognitive":"...","illegal":"..."},"note":"<一句話結論>","timestamp":"<ISO 時間>"}' >> scripts/jobs/JOB-229/_spot_check.log
```

把 `...` 替換成實際結果。`note` 寫一句話結論（如「結構與黃金樣本一致，reason 全引用題幹」或「編碼分布偏斜：INc-Ⅱ-6 占 35%」）。
