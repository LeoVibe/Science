#!/usr/bin/env python3
"""
JOB-229 Phase B：跨全量 123 份自然科 JSON 驗證編碼合法性、階段、重複。
讀目錄：_golden_samples/* (1) + 三下_自然_pilot/* (5) + 三下_自然_*出版社/* (117)
寫報告：knowledge/3_考古題/3_L2_結構化抽取/_validation_report_natural.json
不修改原始 JSON。
"""
...
EXPECTED_FILES = 123  # 1 黃金 + 5 Pilot + 117 全量
...
REPORT_PATH = os.path.join(DATA_ROOT, "_validation_report_natural.json")

INPUT_GLOBS = [
    os.path.join(DATA_ROOT, "_golden_samples/翰林_112_成功國小_第一次段考.json"),  # 不掃 _archive_social
    os.path.join(DATA_ROOT, "三下/三下_自然_pilot/*.json"),
    os.path.join(DATA_ROOT, "三下/三下_自然_翰林/*.json"),
    os.path.join(DATA_ROOT, "三下/三下_自然_康軒/*.json"),
    os.path.join(DATA_ROOT, "三下/三下_自然_南一/*.json"),
]
