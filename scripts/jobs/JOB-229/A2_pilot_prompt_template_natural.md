# JOB-229 Phase 4 Codex Pilot Prompt 模板

> 此模板由 dispatch 腳本注入 `{EXAM_ID}` / `{MD_PATH}` / `{OUTPUT_PATH}` 後送給 codex exec。
> 派工單依據：`jobs/JOB-229-AG-G3S2-自然-考古題L2結構化抽取.md`
...
你是 Codex，負責對一份三下_自然 整合版 MD 試卷做 **Phase A1 + A2 整合抽取**：
...
```json
{
  "_meta": {
    ...
    "schema_version": "1.0",
    "extracted_at": "<ISO 時間>",
    "extractor": "Codex (gpt-5.5) - JOB-229 Pilot",
    "_inheritance": {
...
2. **❌ 禁止使用其他學習階段（Ⅰ/Ⅲ/Ⅳ/Ⅴ）的編碼。** 三下_自然 = 第 Ⅱ 學習階段，code 必須以 `-Ⅱ-` 中綴。
