# JOB-228 Phase 4 Codex Pilot Prompt 模板

> 此模板由 dispatch 腳本注入 `{EXAM_ID}` / `{MD_PATH}` / `{OUTPUT_PATH}` 後送給 codex exec。
> 派工單依據：`jobs/JOB-228-AG-G3S2-社會-考古題L2結構化抽取.md`
...
你是 Codex，負責對一份三下_社會 整合版 MD 試卷做 **Phase A1 + A2 整合抽取**：
...
- `黃金樣本（schema 範例）`：
  - `knowledge/3_考古題/3_L2_結構化抽取/_golden_samples/翰林_108_文德國小_第二次段考.json`（dual_source 範例）
  - `knowledge/3_考古題/3_L2_結構化抽取/_golden_samples/康軒_111_新北安和國小_期中考.json`（codex_only 範例 + 8 條 schema 觀察）
...
```json
{
  "_meta": {
    ...
    "schema_version": "1.0",
    "extracted_at": "<ISO 時間>",
    "extractor": "Codex (gpt-5-codex) - JOB-228 Phase 4 Pilot",
    "_inheritance": {
...
1. **❌ 禁止使用不在 35 條合法清單的 code。** 即使原始 MD hint 中出現 `Ae-Ⅱ-1` 等，也只算 OCR 錯位，不在合法清單。
2. **❌ 禁止使用其他學習階段（Ⅰ/Ⅲ/Ⅳ/Ⅴ）的編碼。** 三下_社會 = 第 Ⅱ 學習階段，code 必須以 `-Ⅱ-` 中綴。
3. **✅ 找不到合適編碼時**，留 `codes_candidate: []` 並在題目欄加 `"extract_skipped": "no_matching_code"`，**不可硬塞**。

### Reason 品質

4. **❌ 禁止寫「相關」「對應」「與此題有關」等空泛字眼。** 必須引用題幹原句。
5. **✅ 範例（合格）**：`"題幹「我們可以從『臺灣行政區圖』找到...」直接對應「居民的生活方式與空間利用」對居住地方的空間定位。"`
6. **❌ 範例（不合格）**：`"與此題相關"`、`"考居住地方知識"`。
...
4. 對每題從 35 條中找 1-3 條最匹配的 code（先看主題：人地/消費/文化/變遷/選擇/公共事務）
5. 寫 `_summary` 統計
6. 自查清單跑一遍，發現違規修正
