# JOB-240 Phase 0.3 Codex Pilot Prompt 模板（五下_國語）

> 此模板由 dispatch 腳本注入 `{EXAM_ID}` / `{MD_PATH}` / `{OUTPUT_PATH}` 後送給 codex exec。
> 派工單依據：`jobs/JOB-240-AG-五下-國語-考古題L2結構化抽取.md`
...
你是 Codex，負責對一份五下_國語 整合版 MD 試卷做 **Phase A1 + A2 整合抽取**：
...
- `黃金樣本（唯一 schema 範例）`：
  - **只能讀**：`knowledge/3_考古題/3_L2_結構化抽取/_golden_samples/五下_國語_翰林_*.json`（國語五下主黃金樣本，Claude Sonnet 4.6 親做、編碼合法率 100%）
  ...
  - **❌ 嚴禁參考**：任何社會科或自然科黃金樣本
  - **❌ 嚴禁參考**：任何 `-Ⅲ-` 或 `-Ⅰ-` 或 `-Ⅳ-` 或 `-Ⅴ-` 編碼（五下使用第 Ⅲ 學習階段）
  - 國語科 code 格式**僅限**：
  ...
  - **若你看到 `-Ⅰ-` 或 `-Ⅲ-` 中綴 → 立即停止，五下國語必須用 `-Ⅲ-` 中綴**
...
```json
{
  "_meta": {
    ...
    "schema_version": "1.0",
    "extracted_at": "<ISO 時間>",
    "extractor": "Codex (gpt-5.5) - JOB-240 Pilot",
    "_inheritance": {
...
2. **❌ 禁止使用其他學習階段（Ⅰ/Ⅲ/Ⅳ/Ⅴ）的編碼。** 五下_國語 = 第 Ⅲ 學習階段，code 必須以 `-Ⅲ-` 中綴。
...
2. 解析 MD frontmatter 寫入 `_meta`（注意 semester 為「五下」、subject 為「國語」）
3. 解析「## 試卷」與「## 答案」段落，逐題抽結構化
4. 對每題從 65 條中找 1-3 條最匹配的 code（五下_國語主題：注音符號/字形辨識 / 詞語理解與應用 / 句型與造句 / 閱讀理解與段落大意 / 修辭與文學表現 / 標點符號 / 寫作與語文應用）
5. 寫 `_summary` 統計
6. 自查清單跑一遍，發現違規修正
