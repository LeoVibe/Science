#!/usr/bin/env python3
"""
JOB-240 Phase B：跨全量 121 份五下_國語 JSON 驗證編碼合法性、階段、重複。
讀目錄：_golden_samples/<本 JOB 黃金>.json (1) + 五下_國語_pilot/* (5) + 五下_國語_*出版社/* (~115)
寫報告：knowledge/3_考古題/3_L2_結構化抽取/_validation_report_chinese_g5.json
不修改原始 JSON。
"""
...
EXPECTED_FILES = 115  # 1 黃金 + 5 Pilot + 109 全量
EXPECTED_LEGAL_CODES = 65  # chinese_codes_legal_III.json（31 performance + 34 content）
STAGE_MARK = "Ⅲ"
DETAIL_LIMIT = 30

ROOT = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../.."))
DATA_ROOT = os.path.join(ROOT, "knowledge/3_考古題/3_L2_結構化抽取")
LEGAL_CODES_PATH = os.path.join(DATA_ROOT, "_meta/chinese_codes_legal_III.json")
REPORT_PATH = os.path.join(DATA_ROOT, "_validation_report_chinese_g5.json")

INPUT_GLOBS = [
    os.path.join(DATA_ROOT, "_golden_samples/五下_國語_*.json"),
    os.path.join(DATA_ROOT, "五下/五下_國語_pilot/*.json"),
    os.path.join(DATA_ROOT, "五下/五下_國語_翰林/*.json"),
    os.path.join(DATA_ROOT, "五下/五下_國語_康軒/*.json"),
    os.path.join(DATA_ROOT, "五下/五下_國語_南一/*.json"),
]
