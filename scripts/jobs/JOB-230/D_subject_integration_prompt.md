# JOB-230 Phase D：四下_社會全科目整合 _L2_整合.md 派工 Prompt
...
請以三份版本級摘要為主，撰寫全科目整合分析。範本對照 `三下/三下_社會_L2_整合.md`（JOB-228 結案產出）與 `三下/三下_自然_L2_整合.md`（JOB-229 結案產出）。
...
---
`last_updated`: <ISO 時間>
`updated_by`: Codex (gpt-5.5) - JOB-230 Phase D
`scope`: 四下_社會（翰林+康軒+南一）
`total_files`: 137
`total_questions`: <int>
`total_codes`: <int>
---
...
本檔以三份版本級 `_L2_summary.md` 為主，並以 `_validation_report_social_g4.json` 中四下社會三版本的 `per_file` 切片核對總題數與 codes 數。
