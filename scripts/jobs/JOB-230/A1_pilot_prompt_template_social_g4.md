# JOB-230 Phase 4 Codex Pilot Prompt 模板（四下_社會）

> 此模板由 dispatch 腳本注入 `{EXAM_ID}` / `{MD_PATH}` / `{OUTPUT_PATH}` 後送給 codex exec。
> 派工單依據：`jobs/JOB-230-AG-G4S2-社會-考古題L2結構化抽取.md`
...
你是 Codex，負責對一份四下_社會 整合版 MD 試卷做 **Phase A1 + A2 整合抽取**：
...
- `EXAM_ID`：`{EXAM_ID}`
- `MD 整合檔`：`{MD_PATH}`
- `合法編碼清單`：`knowledge/3_考古題/3_L2_結構化抽取/_meta/social_codes_legal_II.json`（35 條：15 學習表現 + 20 學習內容）
- `黃金樣本（唯一 schema 範例）`：
  - **只能讀**：`knowledge/3_考古題/3_L2_結構化抽取/_golden_samples/四下_社會_翰林_108_安和國小_第二次段考.json`（社會四下主黃金樣本，50 題、88 codes、24/35 distinct codes 覆蓋、編碼合法率 100%、Claude Opus 4.7 親做）
  - **❌ 嚴禁參考**：`knowledge/3_考古題/3_L2_結構化抽取/_meta/science_codes_legal_II.json`（自然科編碼清單，與本任務無關）
  - **❌ 嚴禁參考**：`_golden_samples/翰林_112_成功國小_第一次段考.json` 等任何自然科黃金樣本（code prefix 如 ti/tr/po/INa 等不適用社會科）
  - 社會科 code prefix **僅限**：
    - performance（學習表現）：`1a / 1b / 1c / 2a / 2b / 2c / 3b / 3c` 共 8 個，全部以 `-Ⅱ-` 中綴
    - content（學習內容）：`Aa / Ab / Ac / Ad / Ba / Bb / Bc / Ca / Cb / Cc / Da / Db / Dc` 共 13 個，全部以 `-Ⅱ-` 中綴
  - **若你看到 `ti / tr / tc / tm / po / pe / pa / pc / ai / ah / an / INa / INb / INc / INd / INe / INf / INg` 等 prefix → 立即停止並重讀 social_codes_legal_II.json**
...
```json
{
  "_meta": {
    ...
    "schema_version": "1.0",
    "extracted_at": "<ISO 時間>",
    "extractor": "Codex (gpt-5.5) - JOB-230 Pilot",
    "_inheritance": {
...
1. **❌ 禁止使用不在 35 條合法清單的 code。** 社會科第 Ⅱ 階段合法 prefix 為 performance（1a/1b/1c/2a/2b/2c/3b/3c）+ content（Aa/Ab/Ac/Ad/Ba/Bb/Bc/Ca/Cb/Cc/Da/Db/Dc）。其他 prefix 一律不合法。
2. **❌ 禁止使用其他學習階段（Ⅰ/Ⅲ/Ⅳ/Ⅴ）的編碼。** 四下_社會 = 第 Ⅱ 學習階段，code 必須以 `-Ⅱ-` 中綴。
3. **✅ 找不到合適編碼時**，留 `codes_candidate: []` 並在題目欄加 `"extract_skipped": "no_matching_code"`，**不可硬塞**。

### Reason 品質

4. **❌ 禁止寫「相關」「對應」「與此題有關」等空泛字眼。** 必須引用題幹原句。
5. **✅ 範例（合格）**：`"題幹「臺灣早期移民從中國閩粵渡海來臺」直接對應「臺灣的歷史文化發展」的內容。"`
...
- [ ] questions[] 數 == MD 試卷部分題目數
- [ ] 全部 codes_candidate.code 在 35 條合法清單內（檢查 social_codes_legal_II.json）
...
1. 讀 `{MD_PATH}` 與 `social_codes_legal_II.json`
2. 解析 MD frontmatter 寫入 `_meta`
3. 解析「## 試卷」與「## 答案」段落，逐題抽結構化
4. 對每題從 35 條中找 1-3 條最匹配的 code（先看主題：人與群體 / 人與環境 / 居住地方 / 臺灣早期歷史 / 地理位置與地形 / 公共事務 / 經濟生活 / 文化多樣性）
5. 寫 `_summary` 統計
6. 自查清單跑一遍，發現違規修正
