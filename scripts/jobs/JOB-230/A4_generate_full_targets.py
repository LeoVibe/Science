#!/usr/bin/env python3
"""JOB-230 Phase 5 全量 targets 產生器
從 137 份四下_社會整合 MD 中扣除 1 黃金 + 5 Pilot = 131 份分到 A/B/C 三 worker。
...
# Phase 0.2 黃金樣本（Claude 親做、編碼 100% 合法、50 題、24/35 distinct codes）
GOLDEN = ['翰林_108_安和國小_第二次段考']
# Phase 0.3 Pilot 5 份（codex 並行、編碼 100% 合法、Spot Check 5/5 PASS）
PILOT = [
    '翰林_112_新北安和國小_期末考',
    '康軒_110_東芳國小_第一次段考',
    '南一_112_勝利國小_期末考',
...
EXCLUDE = set(GOLDEN + PILOT)

OUT_BASE = 'knowledge/3_考古題/3_L2_結構化抽取/四下'
...
def main():
    targets = []
    for pub in ['翰林','康軒','南一']:
        pub_dir = f'knowledge/3_考古題/2_MD淬鍊文字_整合版/四下/四下_社會_{pub}'
        for f in sorted(glob.glob(f'{pub_dir}/*.md')):
            ...
            if not fm:
                ...
                continue
            output_path = f'{OUT_BASE}/四下_社會_{pub}/{exam_id}.json'
            targets.append({
                'exam_id': exam_id,
                'publisher': pub,
                'md_path': f,
    ...
    for w in ['A','B','C']:
        out = {
            '_meta': {
                'phase': f'JOB-230 Phase 5 - 全量 worker {w}',
                'worker': w,
                'created_at': now,
                'total': len(workers[w]),
                'purpose': f'JOB-230 Phase 5 派 codex worker {w} 跑（總 {len(targets)} 份分 A/B/C 三 worker）'
            },
            'targets': workers[w]
        }
        out_path = f'scripts/jobs/JOB-230/_full_targets_{w}.json'
