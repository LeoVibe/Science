#!/usr/bin/env python3
"""
JOB-230 Phase B：跨全量 137 份四下_社會 JSON 驗證編碼合法性、階段、重複。
讀目錄：_golden_samples/<本 JOB 黃金>.json (1) + 四下_社會_pilot/* (5) + 四下_社會_*出版社/* (~131)
寫報告：knowledge/3_考古題/3_L2_結構化抽取/_validation_report_social_g4.json
不修改原始 JSON。
"""
...
EXPECTED_FILES = 137  # 1 黃金 + 5 Pilot + 131 全量
...
REPORT_PATH = os.path.join(DATA_ROOT, "_validation_report_social_g4.json")

# 黃金樣本路徑於 Phase 0.2 確定後填入
INPUT_GLOBS = [
    os.path.join(DATA_ROOT, "_golden_samples/四下_社會_*.json"),  # Phase 0.2 產出後 glob 自動命中
    os.path.join(DATA_ROOT, "四下/四下_社會_pilot/*.json"),
    os.path.join(DATA_ROOT, "四下/四下_社會_翰林/*.json"),
    os.path.join(DATA_ROOT, "四下/四下_社會_康軒/*.json"),
    os.path.join(DATA_ROOT, "四下/四下_社會_南一/*.json"),
]
