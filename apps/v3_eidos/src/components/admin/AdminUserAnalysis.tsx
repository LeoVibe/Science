...
import { BarChart3, ChevronDown, ChevronRight, ExternalLink, Info, Pencil } from 'lucide-react';
...
function formatWrongRatio(d: UserAnalysisDevice): string {
  const r = d.answerWrongRatio;
  if (typeof r === 'number' && Number.isFinite(r)) {
    return `${Math.round(r * 1000) / 10}%`;
  }
  const total = d.answerQuestionTotal;
...
export default function AdminUserAnalysis() {
  ...
  const { getLabel, setLabel } = useAdminDeviceLabels();
  ...
  const openLabelDialog = useCallback(
    (deviceId: string) => {
      setLabelTargetId(deviceId);
      setLabelDraft(getLabel(deviceId) ?? '');
  ...
  const saveLabel = useCallback(() => {
    ...
    setLabel(labelTargetId, labelDraft);
    setLabelDialogOpen(false);
    setLabelTargetId(null);
  }, [labelDraft, labelTargetId, setLabel]);
    ...
    <TooltipProvider delayDuration={200}>
      <div className="space-y-4 animate-in fade-in duration-200">
        <div className="bg-card rounded-2xl border p-4 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                僅列出活躍天數達門檻的裝置；依日曆日與 UTC 小時彙整活動時段、Edge 記錄的來源 IP、日誌中推斷的年級與常點科目。錯誤比例來自活動日誌中{' '}
                <code className="text-[10px] bg-muted px-1 rounded">answer_question</code> 的對錯統計；錯題統計連結依最近一次完整脈絡產生。裝置註記僅存於本機瀏覽器。
              </p>
            </div>
              ...
              <div className="rounded-xl border border-border/60 bg-muted/20 p-3 space-y-1">
                ...
                {summary.topGradeSubject30d ? (
                  <p className="text-muted-foreground">
                    {summary.topGradeSubject30d.grade} 年級 · {summary.topGradeSubject30d.subject}（
                    {summary.topGradeSubject30d.events} 次）
                  </p>
                ) : (
                ...
                <ol className="list-decimal list-inside text-muted-foreground space-y-0.5">
                  {summary.activeDaysRank.slice(0, 5).map((r) => (
                    <li key={r.deviceId}>
                      <span
                        className={
                          getLabel(r.deviceId)?.trim()
                            ? 'text-foreground font-medium text-[10px]'
                            : 'font-mono text-[10px]'
                        }
                      >
                        {getDeviceDisplayPrimary(r.deviceId, getLabel)}
                      </span>{' '}
                      · {r.activeDays} 天
                    </li>
                  ))}
          ...
          {!loading && !error && devices.length === 0 && (
            <div className="py-8 text-center text-muted-foreground text-sm space-y-2">
              ...
              <p className="text-xs">
                本機請先在前台操作幾次（觸發活動同步），或把上方門檻調為 <strong className="text-foreground">1</strong>{' '}
                再按「重新整理」。
                若完全沒有 KV 資料，請確認 Worker 已啟動且{' '}
                <code className="text-[10px] bg-muted px-1 rounded">VITE_API_URL</code> 指向該 Worker。
              </p>
            </div>
          )}
          {!loading && !error && devices.length > 0 && (
            <div className="overflow-x-auto space-y-2">
              {devices.map((d, idx) => {
                ...
                const primary = getDeviceDisplayPrimary(d.deviceId, getLabel);
                const hasNote = Boolean(getLabel(d.deviceId)?.trim());
                return (
                  <div key={d.deviceId} className="rounded-xl border border-border/60 overflow-hidden">
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-2 text-xs items-start">
                            ...
                            <Tooltip>
                              <TooltipTrigger asChild>
                                <span
                                  className={
                                    hasNote
                                      ? 'text-[11px] font-medium text-foreground cursor-default max-w-[12rem] truncate'
                                      : 'font-mono text-[11px] text-foreground cursor-default max-w-[12rem] truncate'
                                  }
                                >
                                  {primary}
                                </span>
                              </TooltipTrigger>
                              <TooltipContent side="top" className="max-w-[min(90vw,28rem)] break-all font-mono text-xs">
                                {d.deviceId}
                              </TooltipContent>
                            </Tooltip>
                            <Popover>
                              <PopoverTrigger asChild>
                                <button
                                  ...
                                  aria-label="顯示完整裝置 ID"
                                  onClick={(e) => e.stopPropagation()}
                                >
                                  <Info className="w-3.5 h-3.5" />
                                </button>
                              </PopoverTrigger>
                              <PopoverContent className="w-auto max-w-[min(90vw,24rem)] text-xs" align="start">
                                <p className="font-semibold text-foreground mb-1">完整裝置 ID</p>
                                <p className="font-mono break-all text-muted-foreground">{d.deviceId}</p>
                              </PopoverContent>
                            </Popover>
                            <button
                              type="button"
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="flex flex-wrap gap-1">
                            {d.topGrade != null ? (
                              <Badge variant="outline">{d.topGrade} 年級</Badge>
                            ) : (
                            ...
                            {d.topSubject ? (
                              <Badge variant="outline">{d.topSubject}</Badge>
                            ) : (
                              <Badge variant="outline" className="text-muted-foreground">
                                科目 —
                              </Badge>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <Badge variant="outline" className="font-mono font-normal break-all text-left max-w-full">
                            {d.clientIps.length ? d.clientIps.join(', ') : '—'}
                          </Badge>
                        </div>
                        ...
                        <div className="space-y-1.5">
                          ...
                          <div className="flex flex-wrap items-center gap-1.5">
                            <Badge
                              ...
                              title={
                                typeof d.answerQuestionTotal === 'number'
                                  ? `答題 ${d.answerQuestionTotal} 筆，錯 ${d.answerQuestionWrong ?? 0} 筆`
                                  : undefined
                              }
                            >
                              錯誤比例 {formatWrongRatio(d)}
                            </Badge>
                            {d.statsWrongPath ? (
                              <a
                                ...
                                className="inline-flex items-center gap-1 rounded-full border border-border bg-background px-2 py-0.5 text-primary font-medium text-[11px] hover:bg-muted/60"
                                onClick={(e) => e.stopPropagation()}
                              >
                              ...
                              <span className="text-[11px] text-muted-foreground">無法推斷統計路徑</span>
                            )}
                          </div>
                        </div>
                      </div>
                    ...
                    {open && (
                      <div className="px-3 py-3 text-xs border-t border-border/50 space-y-3 bg-background">
                        <p className="text-muted-foreground">
                          初次 {formatDate(d.firstSeen)} · 最後 {formatDate(d.lastSeen)} · 時段以 UTC 小時分桶（與前端 ISO
                          紀錄一致）
                        </p>
        ...
        <Dialog open={labelDialogOpen} onOpenChange={setLabelDialogOpen}>
          <DialogContent className="sm:max-w-md" aria-describedby={labelDialogDescId}>
            <DialogHeader>
              <DialogTitle>裝置註記（本機）</DialogTitle>
              <DialogDescription id={labelDialogDescId}>
                僅在此瀏覽器儲存，方便對照「營運統計」與高活躍裝置。留空並儲存可清除註記。
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-2">
              <Label htmlFor="admin-device-label">顯示名稱</Label>
              <Input
                id="admin-device-label"
                value={labelDraft}
                onChange={(e) => setLabelDraft(e.target.value)}
                placeholder="例如：教室 A、家長手機"
                maxLength={64}
              />
              {labelTargetId ? (
                <p className="text-[10px] text-muted-foreground font-mono break-all">ID：{labelTargetId}</p>
              ) : null}
            </div>
            <DialogFooter>
              <Button type="button" variant="secondary" onClick={() => setLabelDialogOpen(false)}>
                取消
              </Button>
