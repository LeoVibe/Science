...
import {
  ...
  getOrCreateUserId, fetchAndMergeUserProfile, syncUserProfileToApi, isShortcutEnabled,
} from '@/utils/storage';
...
const Index = () => {
  ...
  useEffect(() => {
    ...
    setShowSetup(true);
    setProfileReady(true);
  }, [gp, sp, semp, pp, vp]); // 依 URL params 變動同步 state（含切換出版社）
  ...
  useEffect(() => {
    ...
    fetchAndMergeUserProfile(userId).then(merged => {
      if (merged) {
        ...
        setPublisher(merged.publisherBySubject?.[subject] ?? '南一');
      }
    });
  }, [settingsLoaded, subject]);
  ...
  }, [grade, subject, semester, publisher, view, subTab, profileReady, navigate]);
  ...
  useEffect(() => {
    ...
    // 如果是 review/quiz 需完整題目；僅在「已載過同一組合」時略過，避免點題庫總覽連結後沿用錯的題庫
    const needFull = ['review', 'quiz'].includes(view);
    ...
    return () => { cancelled = true; };
  }, [grade, subject, semester, publisher, view, profileReady]);
  ...
  }, [grade, subject, semester, publisher, loaded.questions.length]);
  ...
  }, [grade, subject, semester, publisher, ensureQuestionsLoaded]);
