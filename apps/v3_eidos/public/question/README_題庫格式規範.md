...
## 9. 🤖 系統與 AI 命題防呆總綱 (Authoring Guidelines)

為了避免為製造陷阱而產出極不自然、違反常識的題目，系統與 AI 命題時必須恪守以下平衡準則：

### 9.1 命題心法與角色界線
1. **隱蔽性誘答 (Invisible Traps)**：陷阱的「刻意性」只能存在於**選項的邏輯設計**中（例如針對 L3 素材指出的學生迷思，刻意安插對應的錯誤選項），絕對不能污染**前端題幹情境**。
2. **語境絕對自然 (Contextual Naturalness)**：題幹情境必須完全符合「真實生活」。不能為了配合陷阱或選項，而硬湊出人類在現實生活中不會出現的對話或故事邏輯。
3. **角色切換機制**：構思題幹時，請扮演「擅長說故事的生活家」（語氣平實自然）；設計選項與誘答時，請切換為「嚴謹的教育心理分析師」（精準佈局認知地雷）。

### 9.2 猜題防呆機制 (Anti-Guessing Audit)
- **選項等長原則**：正確與錯誤選項的文字長度、文法結構必須高度一致。（此點將被 `evaluate_question_quality.js` 嚴格檢驗）
- **零常識作答**：題目必須依賴「閱讀文本或語境」才能作答。若學生不看題目、單靠生活道德常識即可選出正解，即為無效廢題。
- **引經據典的誘答**：錯誤選項（Trap）嚴禁湊數。所有 Trap 必須建基於 Level 3 文件中所記載的確切「學生痛點與迷思」。
