        ...
        <div v-for="question in categoryQuestions" :key="question.id" class="review-card">
          ...
          <div v-if="question.options && question.options.length > 0" class="review-options">
            <div
              ...
              :key="index"
              :class="['review-option', { correct: index === (question.answer !== undefined ? question.answer : question.correctAnswer) }]"
            >
              {{ String.fromCharCode(65 + index) }}. {{ option }}
            </div>
          </div>
          <div v-else class="review-answer">
            <strong>答案：</strong>{{ question.answer !== undefined ? question.answer : question.correctAnswer }}
          </div>
...
const categoryQuestions = computed(() => {
  if (!selectedCategory.value || !questionsModule.value) {
    return allQuestions.value // 如果没有选择分类，显示所有题目
  }
