...
- [ ] **建置**：已執行 `npm run build` 且成功（產出在 `dist/`）。
- [ ] **Base 路徑**：`vite.config.js` 中 `base: '/Science/'`（production），對應網址 `https://leovibe.github.io/Science/`。
- [ ] **題庫**：`public/questions/platform/` 已包含要上線的題庫，建置後會一併複製到 `dist/questions/platform/`。
- [ ] **GitHub**：遠端為 `https://github.com/LeoVibe/Science.git`，分支 `main`。
- [ ] **Pages 來源**：Repo 設定 → Pages → Source 選 **GitHub Actions**。
- [ ] 推送後到 **Actions** 確認「Deploy to GitHub Pages」成功，約 1–2 分鐘後造訪：**https://leovibe.github.io/Science/**。
...
```bash
cd /Users/s389080/Documents/文件\ -\ NM389080/work/0_AI_Project/ScienceQuest
