# JOB-067: G6_S2_自然_L4_題庫全規模產出 (大規模擴產版)
...
3. **高階認知 (L4+)**：邏輯推論題與場景遷移題比例必須維持 40% 以上。
...
2. **品質評核**：執行 `node scripts/evaluate_question_quality.js [檔案]`。
   - **DoD 標準**：平均 CQI ≥ 8.0 且 QG 等級達 **L4 (高品質)**。
3. **索引與統計**：
...
## 執行指令 (For Agent)
```bash
請詳細閱讀 `jobs/JOB-067-USER-G6-S2-自然-L4-題庫全規模產出.md` 並執行。
