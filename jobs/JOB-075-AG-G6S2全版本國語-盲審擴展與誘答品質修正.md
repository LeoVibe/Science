...
依據前案（JOB-073）針對「G6 S2 康軒國語」進行之 CQI v2 盲審實測結論，雖然盲測 Match 率高達 100%，但也確實驗證了目前高品質 QL4 題庫存在的兩大潛在風險：
...
### 階段三：確保選項機率公平性
- [ ] 調用 `auto_balance_json.js` 對全版本 JSON 執行選項打散，避免 `QL1 (BIAS)` 選項集中警告。
