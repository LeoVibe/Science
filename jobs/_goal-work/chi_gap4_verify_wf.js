export const meta = {
  name: 'g4-chi-gap4-verify',
  description: '四下國語3課補4題盲測+judge(課文必要錨點)',
  phases: [{ title: '補題雙盲', detail: '盲測員+judge,課文為必要錨點' }],
}

const lessons = [
  { d: 'HanLin', pn: '翰林', n: 7, title: '棒球英雄夢', nq: 1 },
  { d: 'HanLin', pn: '翰林', n: 11, title: '最後一片葉子', nq: 2 },
  { d: 'HanLin', pn: '翰林', n: 12, title: '閱讀課', nq: 1 },
]
...
function blindPrompt(L) {
  return `你是國小四年級國語科專家,盲測作答「${L.title}」(${L.pn}版)補題。
讀取(無標示答案)：jobs/_goal-work/G4_CHI_${L.d}/L${L.n}_gap4_blind.json,共${L.nq}題(idx 0~${L.nq - 1})。
...
讀取：完整題(含正解) jobs/_goal-work/G4_CHI_${L.d}/L${L.n}_gap4_shuffled.json；單課研究紀錄(含課文全文RC-01) knowledge/1_課綱研究/國語/四下/${L.pn}/KL4_四下_${L.pn}_L${L.n}_${L.title}_單課研究紀錄.md；考古題 knowledge/1_課綱研究/國語/四下/${L.pn}/KL4_四下_${L.pn}_L${L.n}_${L.title}_考古題與討論.md。共${L.nq}題。

【國語專用嚴格判準——課文是必要錨點】
belongs=true 唯一條件：題目具體引用本課課文的情節/人物/意象/字詞，換成別課課文就答不出來。
match=true：標示正解正確無誤、非格式崩壞、**選項語意各自獨立不重疊**、**不可憑字面情感色彩(僅一個正向詞)猜中而不必讀課文**。
...
const results = await parallel(lessons.map(L => () =>
  parallel([
    () => agent(blindPrompt(L), { label: `g4-blind:${L.d}_L${L.n}`, phase: '補題雙盲', model: 'sonnet', schema: BLIND_SCHEMA }),
    () => agent(judgePrompt(L), { label: `g4-judge:${L.d}_L${L.n}`, phase: '補題雙盲', model: 'sonnet', schema: JUDGE_SCHEMA }),
