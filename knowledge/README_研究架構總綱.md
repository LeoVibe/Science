...
**文件定位**：本文件為 Eidos 專案的研究工作指導手冊（KL1-KL4 & RM0-RM4）。
...
每一課或每一單元的研究進度，以 RM 代碼標示其「知識探勘」的深度。**2026-06-16 v2 改版：課文非必要門票——考古題與課文為對等的實證素材**（見 `docs/superpowers/specs/2026-06-16-題庫評估規則v2-課文非必要-design.md`）：

| 階段 | 名稱 | 達成條件（研究視角） | 該課題目最高可達 QL |
|:--|:--|:--|:--:|
| RM0 | 骨架期 | 僅有課名／課綱關鍵字，連題都還沒出。 | QL1（禁止出題） |
| RM1 | 常識期 | 僅憑學科常識能出題（無課綱細項、無考古、無課文）。 | QL1 |
| RM2 | 課綱期 | 有課綱對應的課程元素，能確認「這課要教什麼」。 | QL2 |
| RM3 | 實證期 | RM2 ＋（考古題 **或** 課文之一）——二者地位對等，有任一個即達。 | QL4（需盲測通過） |
| RM4 | 完備期 | RM2 ＋ 考古題 ＋ 課文 **都齊**（超額素材）。 | QL4（需盲測通過） |

> **課文非必要**：課文從 v1 的「RM1 必要門票」降為 RM3／RM4 的實證素材之一，**與考古題地位對等**。有考古題或課文任一個即達 RM3；兩個都有為 RM4。拿不到課文但有考古題的課，仍可達 RM3 → 出 QL3／QL4 題。
>
> **RM → QL 是天花板關係**：RM 決定該課題目的 QL 上限（RM1→QL1、RM2→QL2、RM3/RM4→QL4）。完整 QL 定義請見 `question/README_驗證與盲測準則.md` 第四章。
>
...
> **⚠️ v2 改版調整（2026-06-16）**：上方 RM 階梯已將「課文非必要」入規——**考古題與課文地位對等，有任一個即達 RM3，天花板同為 QL4**。本節的 BCDE 補償方法（加量考古題、雙重盲測、回測驗證）仍是有效的品質實踐，但 β+ 路徑「QL 天花板＝QL3」一條**已由 v2 RM 階梯取代**：無課文但有足量考古題（達 RM3）的課，天花板為 QL4，不再封頂 QL3。下表「QL 天花板」欄以新版 RM 階梯為準。
