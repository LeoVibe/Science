# 考古 — 規範與來源索引 (`knowledge/考古/`)

`last_updated`: 2026-04-21
`updated_by`: Claude Code (claude-opus-4-7)

**本目錄前身**：`knowledge/考古題原檔/`（2026-04-11 JOB-172 建立）。
2026-04-21 JOB-207 重構為 `knowledge/考古/`，導入**三軌結構**（原始 / 淬煉 / 索引）與新存檔原則。
...
3. **出題卡點（Production Gate）**：每課需同時滿足：
   ...
   - （詳見 `knowledge/學習議題研究/無課文情境的考古題補償研究法.md`）
...
6. **智財保護與使用邊界**：
   ...
   - 智財目錄（`knowledge/考古/原始/` + `knowledge/考古/淬煉/*.md`）已列入 `.gitignore`，**禁止上傳至 GitHub**。
...
7. **留存政策（2026-04-21 修訂）**：
   ...
   - 原檔位置：`knowledge/考古/原始/`
   - 原檔副本結構：`knowledge/考古/淬煉/`（結構化 MD）+ `_index.json`（索引）
   - 三軌共存；不因 MD 生成而刪 PDF。
...
## 原文追溯
`knowledge/考古/原始/G3/三下_社會/南一_108_永光國小_第三次段考_試卷.pdf`
```
...
| `knowledge/學習議題研究/無課文情境的考古題補償研究法.md` | β+ 路徑（本目錄的主要用武之地） |
...
### 從 `knowledge/考古題原檔/` 遷移到 `knowledge/考古/`
...
knowledge/考古/原始/
knowledge/考古/淬煉/**/*.md
!knowledge/考古/淬煉/**/_index.json
```

### 引用路徑更新（Q3=A）

搜尋並改寫所有 `knowledge/考古題原檔/` 引用為 `knowledge/考古/`：
- `knowledge/README_研究架構總綱.md` §附錄
- `jobs/JOB-172-*.md`、`jobs/JOB-206-*.md`
- `knowledge/學習議題研究/無課文情境的考古題補償研究法.md`
- 其他 KL4 檔（若有）
