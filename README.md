...
### 🚀 `/dojob` (專案開工與任務派發)
* **使用時機**：當接到新任務，或不知該做什麼準備開始下一階段工作時（例如輸入 `/dojob JOB-012`）。
* **觸發行為**：AI 會掃描待辦清單或讀取指定派工單，自動更新 `prj_status.md` 為進行中，並建立展開的 `task.md` 計畫，完成開工對齊。
...
### 🔄 `/sync` (任務收尾文檔同步)
* **使用時機**：每張工單做完，準備收尾時。
* **觸發行為**：AI 會根據這次的開發，自動更新 `prj_status.md`（劃掉待辦）、將新的 UI 規範寫入 `網站功能規格書.md`，並在 `task_history.md` 加上一筆變更日誌。
