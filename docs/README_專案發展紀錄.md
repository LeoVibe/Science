...
`updated_by`: Claude Code (claude-opus-4-8)（pj_sync：JOB-282 結案——五下國語4課BIAS重鑄，106題全過雙盲；意外揭露3課87題內容與課文脫鉤）
...
| JOB | 簡述 | 狀態 |
|:--|:--|:--|
| JOB-282 | **五下國語4課BIAS重鑄（question_prod+question_verify）** 🎯：JOB-281發現康軒L1、翰林L2/L4/L6「正解嚴格唯一最長」比例42.1%~58.8%（既有缺口，JOB-178於2026-04-12僅憑Match+CQI自動結案時尚無BIAS門檻規則）。比照JOB-277方法重鑄106題誘答，PM獨立複驗（非採信agent自報）確認4課BIAS全數降至0.0%，雙盲驗證（全部106題非抽樣）Match Rate 105/106=99.1%，4課CQI重算全數QL4。**意外重大發現**：judge覆核揭露3課（翰林L2/L4/L6）共87/90題（96.7%）題目內容與實際課文完全脫鉤（測驗捏造情節，如翰林L4測「小明小華救傷鳥」但真正課文是《滿修女採訪記》），規模等同JOB-273在五下國語的重現，部分題目檔案自身的`blind_eval_mismatch`欄位早已標記卻從未處理。已核實此問題非本單造成（題幹vs git HEAD逐字相同），依派工紀律停止擴大範圍、如實回報，交由使用者裁定是否另開單（87題整課重出規模）。見 `jobs/JOB-282-Report.md` | 🟢 DONE |

### 2026-07-19
| JOB | 簡述 | 狀態 |
|:--|:--|:--|
| JOB-281 | **五下國語三版本選項插字清除（question_prod+question_verify）** 🎯：上版前實測發現康軒五下國語選項文字含不通順插入贅字（如「才能無往不利**標記**，」），開單估 10 檔/51 題，實測後範圍擴大至**三版本 19 檔/119 題**（原偵測正則漏判句尾無標點與「內容/數據/描述/標誌」等延伸詞，改逐題通讀後才發現真實規模；並非全部 Gemini-2.5-Pro 課次均受影響，需逐題判讀無法用模型篩選跳過）。額外修復康軒 L11 五處文字硬截斷（含 1 處在正解本身）。雙盲風險分層抽樣（全部截斷題+隨機25題）29/29 Match 100%。**過程事故**：1 個 subagent 因權限限制只寫入紀錄檔未寫入正式檔案卻回報成功，PM 逐檔唯讀複驗後揪出並補寫，另 judge 覆核揪出 3 處殘留（含 1 處既有錯字、1 處既有截斷）已修正。**遺留**：4 課（康軒L1、翰林L2/L4/L6）既有 BIAS 超標（42%~59%，正解本身結構性偏長，與插字無關），規模同 JOB-277，經使用者裁定另開單處理，不阻塞本次上線。見 `jobs/JOB-281-Report.md` | 🟢 DONE |

### 2026-07-18
| JOB | 簡述 | 狀態 |
|:--|:--|:--|
| JOB-280 | **評分腳本加只讀模式，根除「查分數誤改題庫」陷阱（engineering）** 🎯：`evaluate_question_quality.js` 的 `evaluateFile()` 每次呼叫都無條件把 cqi_score/quality_level 寫回來源檔，導致跑統計/查分數就誤改全站題庫（JOB-272/275/276/277/279 連 5 次踩雷、每次還原 200~300 檔）。加 `{dryRun}` 選項，統計腳本與 CLI 查詢改只讀（加 `--write` 才寫回）；計分邏輯零改動、dryRun 與寫回分數逐檔相同、刻意寫回的呼叫端不受影響。順帶修正一個既有錯誤統計（G3_S2 國語南一 libraryStats 330/1→331/0，與題庫檔實況一致）。**遺留**：G3_S2_CHI_NANYI_L3 一題 stored QL4 vs rubric QL1 的次要落差（顯示以 stored 為準，無需動作）。見 `jobs/JOB-280-Report.md` | 🟢 DONE |
| JOB-279 | **G3/G4 遺留題裁定與收尾（question_verify+docs_ops）** 🎯：三組遺留一次收斂——①三下國語翰林 L10/L11 共 8 題 pending 實查全為**文本錯位**廢題（考別課內容、1 題題幹破損），使用者裁定直接刪除，兩課定案 31/33 題全上架、翰林三下國語 404/404 收斂；②三下自然康軒 L1 盲測歧義題（「可修正哪種想法」正解標成正確想法）使用者裁定修題幹＋正解，單題盲測 MATCH 後上架，該課回 50/50；③四下國語 KL4 淬鍊遺留 4 課的本機 _new 重做檔跑 verify_kl4.py 全 FAIL 不可轉正，使用者裁定接受維持 RM1（不影響現行 36/36 QL4 題庫，僅研究資產缺口）。**遺留**：evaluateFile() 副作用第 5 次觸發（265 檔還原），dryRun 修正單建議儘速開立。見 `jobs/JOB-279-Report.md` | 🟢 DONE |
| JOB-278 | **上架交接鏈清算與殘留檔歸位（engineering+docs_ops）** 🎯：全專案盤點後的一次清算——JOB-215/257/259 三張懸置單補結案（實測佐證：三下社會 17 課現況 50/50 全上架 BIAS 全過、康軒/南一實為 6/17 commit `4670b830` 上架、四下社會被 JOB-272 取代）；17 個已轉正 `_new.json` 移出版控；iCloud 衝突副本清除 4,315 個（JOB-275 後 13 天內再生，根因＝專案仍在 iCloud 路徑，搬家需使用者決策）；2 個考古題索引 manifest 入版控；進度彙整三下社會列過期敘述校正。**遺留**：`.env 2.local` 值差異待人工、JOB-262（五下）維持開放暫緩、v2 dist 436 個追蹤中副本（P2）。見 `jobs/JOB-278-Report.md` | 🟢 DONE |
...
| JOB | 簡述 | 狀態 |
|:--|:--|:--|
| JOB-271 | **三下翰林 L10/L11 補題達 ≥30 is_publishable** 🎯：JOB-266 九步校正後 L10/L11 各剩 25 approved，未達上架門檻。Codex gpt-4o 訂閱制各出 8 題 → Claude subagent 九步校正。**結果**：L10 6/8 approved（Q2/Q5 pending：step 3(a) 無課文支持，B612/草不多等情節不在劇本文本中）；L11 8/8 approved（畫龍點睛課文錨點全通過）。BIAS 校正：L10 Q3/Q4 選項拉長（0% BIAS）。正式 JSON 追加（L10 +6→31 pub；L11 +8→33 pub），兩課均達 ≥30 門檻。**🎯 里程碑**：翰林三下國語 404/412 is_publishable；L10/L11 全數達上架標準。見 `jobs/JOB-271-Report.md` | 🟢 DONE |
| GOAL-上版 | **三下國自社完整驗證+上版（9版本2536題QL4）** 🎯：自主執行，把3個風險版本拉到與其他版本同標準。**社會康軒**(6課300題)+**社會南一**(5課250題)：codex重出草稿跑雙盲馬車（打散→盲測Match94-100%→對應考點judge），南一L5《打造幸福的家園》對應僅22/50→codex契約補28題，過閘+補題各課50題QL4（commit d12e4a2b）。**國語康軒**(L1《許願》/L2《下雨的時候》/L3《遇見美如奶奶》文本錯位)：codex扣KL4課文全文重出各35題，課文錨點judge驗證扣課文105/105錯位0、雙盲過閘各32/27/32題QL4，徹底解決原錯位（L1原僅19上架）（commit cd9b87d9）。**最終驗收**：三下國自社9版本全QL4、RM3素材足、audit矛盾0、全課≥25門檻，共2536題。國語康軒L4-L12為Gemini舊驗證、無錯位、達門檻、本次未重驗（風險點僅L1-L3已解決）。數學/英語維持下架。見 `jobs/_goal-work/_GOAL_progress.md` | 🟢 DONE |
| JOB-270 | **三下社會翰林六課重出驗證上架（6/6 課各 50 題 QL4，共 300 題）** 🎯：承 JOB-268 降級的 94 題，將 codex 重出的 300 題 `_new` 草稿跑馬車後半驗證上架。**馬車**：階段2腳本閘（CQI 6.79-7.14、長度BIAS 全無）→ 正解 idx%4 機械循環（4課50/50）**全庫打散** → 階段4 雙盲盲測（sonnet vs 出題 gpt-5.5，原題 Match 94-100%、補題 296/300）→ 階段3 對應考點 judge（各課抽 KL4 考點比對）。**揪出**：L4 整課跑題（共享運輸系統）、L6 整課跑題（交通安全，JOB-268 同問題重演）→ 對應僅 12/3；**雙盲抓出 codex 2 題答案與解析矛盾**（L4-29 高鐵→火車、L6-37 服務性質→節慶）。**補題**：codex 契約式按考點補 119 題（10/8/12/38/4/47），無跑題。各課集滿 50 升 QL4、is_publishable=true，主檔舊題取代撤下。audit：G3_S2_社會_翰林 QL4 RM3 無矛盾。已部署（9d78921c）。遺留：解析一致性宜前置為腳本閘、補題對應僅契約+抽驗。見 `jobs/JOB-270-Report.md` | 🟢 DONE |
