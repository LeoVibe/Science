...
| JOB | 簡述 | 狀態 |
|:--|:--|:--|
| JOB-253 | **康軒自然盲測+上正式機（pilot）— Claude subagent 雙盲流程驗證** 🎯：康軒三下自然 200 題用 Claude subagent 雙盲（出題Codex/盲測Claude，不同模型符合雙盲），4課並行盲判。**Match 199/200（99.5%）**，L2/L3/L4全100%、L1 49/50。唯一Mismatch(L1-id32)診斷為題幹「可修正哪種想法」語意歧義，依準則標pending未自動改答案。回寫QL4(avgCqi 9.20)、is_publishable 49/50/50/50課級全達標(≥25)。**上正式機**：手動只同步康軒5檔至public(未跑全量sync避免殃及翰林/南一未盲測題下架)，git diff驗blast radius僅康軒,push觸發Cloudflare/Pages部署。**正式站mixed-state**:康軒新QL4上線、翰林/南一仍舊版待盲測、About統計刻意未重生(cosmetic)。pilot驗證「subagent盲測+上版」可行,翰林/南一/社會可比照。見 `jobs/JOB-253-Report.md` | 🟢 DONE |
