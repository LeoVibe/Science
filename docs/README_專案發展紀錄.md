...
`updated_by`: Claude Code（JOB-205：JOB-184 批次建檔事故修復）
...
| JOB-205 | **JOB-184 批次建檔事故修復（防破窗 + placeholder 清理）**：追查 `eccb974` mega-commit（標記 JOB-184 但實際涵蓋 5-6 個 JOB 的 4422 檔變更），定位三層防線崩壞：①3 支腳本 fallback 沉默填 `L{N}` ②commit message 隱藏範圍 ③品管 gate 未查 title。階段 2 防破窗：`repair_manifests.js:63`、`auto_generate_questions.js:80`、`job184_g5s2_social_orchestrator.js:147` 三處 fallback 改 throw；`verify_ui_data_integrity.mjs` 加 D-INT-5 規則（warning 模式）；新增 `verify_no_placeholder_title.mjs` 獨立稽核。階段 1 事故文件：`docs/技術設定/JOB-184-批次建檔事故分析.md` 建立。階段 3 補 title：G5 Math S2 × 3 publisher 從 KL4 檔名 parse（30 課）+ G3 ENG S2 × 3 publisher 從發展綱要 §三.1 parse（12 課）= **42 課同步**；27 manifest（~136 課）列遺留待後續研究 JOB。見 `jobs/JOB-205-Report.md` | 🟢 DONE |
